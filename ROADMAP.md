...
| H4 | **Phase3 — 服务端 gRPC（serve() ALPN 分发 + h2 server）** 🔲 | `tcpListen` TLS 加 ALPN `h2`；明文连接嗅探 `PRI * HTTP/2.0` 前导自动切 h2c；服务端 h2 引擎 + gRPC 服务端语义。决策点：HPACK 是否下沉 C 原语。 | 定位必需（用户拍板 2026-09-09：IoT 设备 + 边缘节点双端都需要），推进项 |
