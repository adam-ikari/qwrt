 ...
 *
 *   qz_post_message  → 宿主入站 FIFO（MPSC，宿主线程推）→ loop 线程装信封写通道
 *   message_cb         → loop 线程解信封 → 回调解出 payload（JSON 文本）
 *   qz_wait_idle     → 发 CONTROL{idle} → 主RT 排空后回 ack 并自身退出 → join
 *   qz_destroy       → 三级终止（§9.2）→ 收尸 → join → 释放
 *
 * 线程语义与线程后端一致：宿主 loop 线程独占通道读写，message_cb 仍跑在
 * 「qzjs 线程」（此处 = 宿主 loop 线程），宿主无需改变回调线程假设。
 *
...
/* ── 宿主 loop 线程 ──
 * 只做通道 I/O（JS 在主RT 进程里跑）。与 qz_thread_main 的对应关系：
