/*
 * amoib Thread Main Loop (执行模型 A)
 *
 * am_t 自持一个内部线程：uv_loop_init + wake async 注册，JS 运行时由
 * am_runtime_init 建好（含 polyfill 注入 / 扩展 init / DAP attach），然后
 * 用 mutex+cond 与宿主握手（am_create 阻塞到 thread_ready）。此后线程在
 * uv_run(UV_RUN_ONCE) 里处理事件（wake 回调排空入站 FIFO 并派发 onmessage），
 * 事件间 flush 微任务。destroy 置 shutting_down 并 wake → 线程退出主循环 →
 * am_thread_teardown 清理。
 *
 * PAL 时代"回调在线程上排队、am_tick 回放"的 deferred 队列已删除：现在所有
 * libuv 回调（timer/io/wake）本来就跑在 amoib 线程，直接 JS_Call 即可。
 */
...
/* ── idle detection (am_wait_idle support) ── */
...
static void am_idle_walk_cb(uv_handle_t *h, void *arg)
{
    am_idle_state_t *st = (am_idle_state_t *)arg;
    if (h == (uv_handle_t *)&st->rt->wake) return;   /* exclude the internal wake async */
    /* CTL-2 §2.3：本地控制端点（监听 + 活跃连接）是基础设施句柄，不参与
     * idle 判定——否则开了端点的 runtime（尤其 amoib-ctl 连接期间）永不判
     * idle，wait_idle/destroy 卡死。与 wake async / IPC pipe / DAP timer
     * 同款豁免。 */
    if (am_ctl_endpoint_owns(st->rt, h)) return;
    ...
    if (am_proc_handle_is_pipe(st->rt, h)) return;
#endif
...
{
    if (am_msg_has_pending(rt)) return 0;   /* inbound queue non-empty */
    ...
    st.rt = rt;
    st.busy = 0;
    uv_walk(&rt->loop, am_idle_walk_cb, &st);
    if (st.busy) return 0;
...
/* uv_async callback: runs on the amoib thread; drains the inbound queue and dispatches onmessage */
static void am_wake_cb(uv_async_t *a)
{
    ...
    while ((m = am_msg_pop(rt)) != NULL) {
         ...
         * 执行）；其余（含 M-P3 的 PORT_TRANSFER）走 __am_dispatch__，kind
...
void am_thread_main(void *arg)
{
    ...
    if (uv_loop_init(&rt->loop) != 0) {        rt->ready_err = -1;
        ...
        loop_inited = 1;
        rt->wake.data = rt;
        if (uv_async_init(&rt->loop, &rt->wake, am_wake_cb) != 0) {            rt->ready_err = -1;
        } else if (rt->config.control_plane == AM_CONTROL_LOCAL &&
                   am_ctl_endpoint_init(rt) != 0) {
            /* LOCAL 档端点 bind/listen 失败 → 显式失败（不静默降级为
             * IN_PROC；§5.3 同款：能力缺失就报错，不伪造）。 */
            rt->ready_err = -1;
        } else if (am_runtime_init(rt) != 0) {            rt->ready_err = -1;
        } else if (rt->config.initial_script) {
            char *err = NULL;
            if (am_eval_internal(rt, rt->config.initial_script, &err) != 0) {
                /* 异常：记录 ready_err，握手后线程退出；am_create 返回 NULL */
                free(err);
                rt->ready_err = -1;
            }
        }
    ...
    /* ready handshake: atomic store (host am_create spins until it reads 1).
    ...
    while (!__atomic_load_n(&rt->shutting_down, __ATOMIC_ACQUIRE)) {
        ...
        am_ctl_reap_timeouts(rt);
        if (__atomic_load_n(&rt->shutting_down, __ATOMIC_ACQUIRE)) break;
        am_flush_microtasks(rt);
        if (__atomic_load_n(&rt->wait_idle, __ATOMIC_ACQUIRE) && am_loop_idle(rt)) {
