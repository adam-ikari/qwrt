/*
 * amoib Control Plane — CTL-0 进程内基线
 *
 * 控制命令消息（入队，flags=CONTROL）→ amoib 线程 wake 分流点自主执行。
 * 外部只发「求」，运行时自主完成——与 idle safepoint 同哲学。
 *
 * 命令集（CTL-0 四命令）：eval / inspect / metrics / interrupt。
 * interrupt 唯一无安全点：原子标志投递即生效（§3.9）。
 *
 * 回执表（§1.2）：登记先于入队（dispatch 必能命中）；消费与超时回收 amoib
...
{
    if (rt->worker_self)
        return (int32_t)((am_worker_t *)rt->worker_self)->id;
    return 1;                   /* AM_IPC_MAIN_ID：主RT 通道上的本地标签 */
}
...
static void ctl_emit_remote(am_t *rt, int32_t target,
                            const uint8_t *json, size_t len)
{
    int32_t self = am_ctl_local_id(rt);
    if (target == 0 || target == 1) {
        am_ipc_child_emit(self, target, IPC_ENV_KIND_CONTROL,
                            0,
                            json, (uint32_t)len);
        return;
    }
    for (int i = 0; i < AM_MAX_PROC_HANDLES; i++) {
        ...
        if (h->live && h->proc && h->proc->id == target) {
            am_proc_post(h->proc, self, target, IPC_ENV_KIND_CONTROL,
                           0,
                           json, (uint32_t)len);
            return;
        }
...
static void ctl_emit_remote(am_t *rt, int32_t target,
                            const uint8_t *json, size_t len)
{
    AM_UNUSED(rt); AM_UNUSED(target); AM_UNUSED(json); AM_UNUSED(len);
}
#endif

/* 查表取条目（回程方向 + 端点 sink）并移除（amoib 线程独占；回执消费点）。 */
static int32_t ctl_take_reply(am_t *rt, const char *correl, void **sink_out)
{
    if (sink_out) *sink_out = NULL;
    if (!correl) return -1;
    int32_t dir = -1;
    uv_mutex_lock(&rt->ctl_lock);
    struct am_ctl_recept_s **pp = &rt->ctl_pending;
    while (*pp) {
        if (strcmp((*pp)->correl, correl) == 0) {
            struct am_ctl_recept_s *r = *pp;
            *pp = r->next;
            dir = r->reply_dir;
...
static void ctl_emit_obj(am_t *rt, JSContext *ctx, JSValue obj,
                         int32_t reply_dir, void *sink)
{
    ...
    if (!json) return;
    if (sink) {
        am_ctl_conn_write(sink, json, strlen(json));
    } else if (reply_dir >= 0) {
...
static void ctl_send_receipt(am_t *rt, JSContext *ctx, JSValue receipt_obj)
{
    int32_t reply_dir = -1;
    void *sink = NULL;
...
static void ctl_error_receipt(am_t *rt, JSContext *ctx, char *correl,
...
static void ctl_timeout_receipt(am_t *rt, JSContext *ctx, const char *correl,
                                int32_t reply_dir, void *sink)
{
...
void am_ctl_register(am_t *rt, const char *correl, uint64_t deadline_ns,
                       int32_t reply_dir, void *sink)
{
    struct am_ctl_recept_s *r =
        (struct am_ctl_recept_s *)calloc(1, sizeof *r);
...
static void ctl_unregister(am_t *rt, const char *correl)
{
...
static int ctl_claim(am_t *rt, const char *correl)
{
    if (!correl) return 1;      /* 无 correl：无法核验，放行（回执本就不可配对） */
    uint64_t now = uv_hrtime();
    struct am_ctl_recept_s *dead = NULL;
    int live = 0;
    uv_mutex_lock(&rt->ctl_lock);
    struct am_ctl_recept_s **pp = &rt->ctl_pending;
    while (*pp) {
...
{
    return am_control_sink(rt, bytes, len, NULL);
}

int am_control_sink(am_t *rt, const char *bytes, size_t len, void *sink)
{
    if (!rt || rt->magic != AM_MAGIC || !bytes)
        return -1;
    /* OFF：控制面恒拒（§4.1 默认档）。 */
    if (rt->config.control_plane == AM_CONTROL_OFF)
        return -1;
    ...
    am_ctl_register(rt, correl, deadline, -1, sink);

    /* ISOLATED 宿主：入队后由 host_wake_cb 装 CONTROL 信封发主RT（target
     * 取自命令 "target" 字段）；THREAD：由 am_wake_cb 就地 dispatch。 */
    int rc = am_msg_push(rt, buf, len, AM_MSG_SRC_HOST,
                           AM_MSG_FLAG_CONTROL);
    free(buf);
    if (rc != 0) {
...
{
    if (target == local_id) return AM_CTL_ROUTE_LOCAL;
    if (target == 0 || target == 1) return AM_CTL_ROUTE_UP;
    if (target > 1) return AM_CTL_ROUTE_DOWN;
    return AM_CTL_ROUTE_DROP;     /* target < 0：无此地址 */
}

/* 本地执行一条命令：登记回执条目（回程 = reply_dir 信封方向 / sink 端点
 * 连接）→ 入 msgq（flags=CONTROL）交 dispatch。interrupt 在入队前置原子
 * 标志（§3.9 投递即生效）。 */
static int ctl_local_command(am_t *rt, const uint8_t *payload, uint32_t len,
                             int32_t reply_dir, void *sink)
{
    char *buf = (char *)malloc((size_t)len + 1);
    if (!buf) return -1;
    ...
    if (op && strcmp(op, "interrupt") == 0)
        __atomic_store_n(&rt->ctl_interrupt, 1, __ATOMIC_RELEASE);
    uint64_t deadline = uv_hrtime() + (uint64_t)timeout_ms * 1000000ULL;
    am_ctl_register(rt, correl, deadline, reply_dir, sink);
    int rc = am_msg_push(rt, buf, (size_t)len, AM_MSG_SRC_HOST,
                           AM_MSG_FLAG_CONTROL);
    free(buf);
...
static int ctl_forward(am_t *rt, int32_t source, int32_t target,
                       const uint8_t *payload, uint32_t len)
{
    ...
    if (target > 1) {
        for (int i = 0; i < AM_MAX_PROC_HANDLES; i++) {
            am_proc_handle_t *h = &rt->proc_handles[i];
            if (h->live && h->proc && h->proc->id == target) {
                am_proc_post(h->proc, source, target,
    ...
    am_ipc_child_emit(source, target, IPC_ENV_KIND_CONTROL, 0,
                        payload, len);
    return 0;
#else
    AM_UNUSED(rt); AM_UNUSED(source); AM_UNUSED(target);
    AM_UNUSED(payload); AM_UNUSED(len);
    return -1;
#endif
}
...
int am_ctl_deliver_receipt(am_t *rt, const uint8_t *payload, uint32_t len)
{
    ...
    if (sink) {
        am_ctl_conn_write(sink, buf, len);          /* 端点控制器 */
    } else if (dir >= 0) {
...
{
    if (!rt || rt->magic != AM_MAGIC || !payload) return -1;
    /* OFF：信封 CONTROL 命令类入站即丢弃（§4.1）。系统级 CONTROL（握手/
     * idle/shutdown）由调用方先行分流，不受本档影响。 */
    if (rt->config.control_plane == AM_CONTROL_OFF) return -1;
    ...
    {
        int32_t p[AM_SELF_PATH_MAX];
        int pn = ctl_extract_path(payload, len, p, AM_SELF_PATH_MAX);
        if (pn > 0) {
            ...
            if (on_path && d == pn) {
                if (ctl_is_receipt(payload, len))
                    return am_ctl_deliver_receipt(rt, payload, len);
                return ctl_local_command(rt, payload, len, source, NULL);
            }
            if (on_path)
                return ctl_forward(rt, source, p[d], payload, len);
            /* 1 = AM_IPC_MAIN_ID（mock 构建不编入 ipc_process.h，用字面量） */
            return ctl_forward(rt, source, 1, payload, len);
        }
    }

    switch (am_ctl_route_decide(local_id, target)) {
    case AM_CTL_ROUTE_LOCAL:
        /* 命中本地的可能是「命令」（下发执行）或「回执」（沿树回程）。 */
        if (ctl_is_receipt(payload, len))
            return am_ctl_deliver_receipt(rt, payload, len);
        return ctl_local_command(rt, payload, len, source, NULL);
    case AM_CTL_ROUTE_UP:
    case AM_CTL_ROUTE_DOWN:
        return ctl_forward(rt, source, target, payload, len);
    case AM_CTL_ROUTE_DROP:
    default:
        return -1;
    }
}
...
int am_control_endpoint_cmd(am_t *rt, const char *bytes, size_t len,
                              void *sink)
{
    if (!rt || rt->magic != AM_MAGIC || !bytes) return -1;
    if (rt->config.control_plane != AM_CONTROL_LOCAL) return -1;
     ...
     * sink 写回。 */
    {
        ...
        if (pn > (int)rt->self_path_len) {
            ...
            am_ctl_register(rt, correl, dl, -1, sink);
            return ctl_forward(rt, am_ctl_local_id(rt),
    ...
    int32_t local = am_ctl_local_id(rt);
    int rc;
    if (target == local) {
        ...
        if (rc != 0) {
            ...
            if (n > 0 && (size_t)n < sizeof err)
                am_ctl_conn_write(sink, err, (size_t)n);
        }
    }
    free(correl);
    return rc;
...
void am_ctl_reap_timeouts(am_t *rt)
{
    uint64_t now = uv_hrtime();
    struct am_ctl_recept_s *dead = NULL;
    struct am_ctl_recept_s **dead_tail = &dead;
...
void am_ctl_teardown(am_t *rt)
{
    ...
    struct am_ctl_recept_s *r = rt->ctl_pending;
    while (r) {
        struct am_ctl_recept_s *next = r->next;
        free(r->correl);
        free(r);
        r = next;
    }
...
/* ── Dispatch（amoib 线程独占，wake safepoint） ── */

/* 目标 ctx：cmd.ctx_id 缺省 → active ctx；提供但不存在 → NULL（NOT_FOUND）。 */
static am_ctx_t *ctl_target_ctx(am_t *rt, JSContext *pctx, JSValue cmd)
{
    ...
    if (!has || id < 0) return am_get_active_ctx(rt);
    return am_get_ctx_by_id(rt, id);
}

static void ctl_eval(am_t *rt, JSContext *rcpt_ctx, JSValue cmd, char *correl)
{
    am_ctx_t *target = ctl_target_ctx(rt, rcpt_ctx, cmd);
...
static void ctl_inspect(am_t *rt, JSContext *rcpt_ctx, JSValue cmd, char *correl)
{
    am_ctx_t *target = ctl_target_ctx(rt, rcpt_ctx, cmd);
    if (!target || !target->jsctx) {
        ctl_error_receipt(rt, rcpt_ctx, correl, "no such context", "NOT_FOUND");
...
static void ctl_metrics(am_t *rt, JSContext *ctx, char *correl)
{
...
{
    /* amoib 线程独占。m->data 为命令 JSON（msgq 保证 NUL 结尾）。 */
    JSContext *ctx = am_get_active_jsctx(rt);
    if (!ctx) return;
    ...
    if (op && strcmp(op, "eval") == 0) {
        ...
        /* 标志已在 am_control（生产者线程）置位并会在引擎指令边界触发；
         * 此处只回执（§3.9 投递即生效）。 */
...
int am_ctl_endpoint_init(am_t *rt) { (void)rt; return -1; }
void am_ctl_endpoint_close(am_t *rt) { (void)rt; }
int am_ctl_endpoint_owns(am_t *rt, void *h) { (void)rt; (void)h; return 0; }
