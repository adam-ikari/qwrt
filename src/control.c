...
/* 查表取条目（回程方向 + 端点 sink）并移除（qwrt 线程独占；回执消费点）。 */
static int32_t ctl_take_reply(qwrt_t *rt, const char *correl, void **sink_out)
{
                         ...
                         int32_t reply_dir, void *sink)
{
    ...
    if (!json) return;
    if (sink) {
        qwrt_ctl_conn_write(sink, json, strlen(json));
    } else if (reply_dir >= 0) {
...
{
    return qwrt_control_sink(rt, bytes, len, NULL);
}
...
int qwrt_control_sink(qwrt_t *rt, const char *bytes, size_t len, void *sink)
{
    ...
    qwrt_ctl_register(rt, correl, deadline, -1, sink);
 ...
 * 标志（§3.9 投递即生效）。 */
static int ctl_local_command(qwrt_t *rt, const uint8_t *payload, uint32_t len,
                             int32_t reply_dir, void *sink)
{
    char *buf = (char *)malloc((size_t)len + 1);
    if (!buf) return -1;
    ...
    if (op && strcmp(op, "interrupt") == 0)
        __atomic_store_n(&rt->ctl_interrupt, 1, __ATOMIC_RELEASE);
    uint64_t deadline = uv_hrtime() + (uint64_t)timeout_ms * 1000000ULL;
    qwrt_ctl_register(rt, correl, deadline, reply_dir, sink);
...
static int ctl_forward(qwrt_t *rt, int32_t source, int32_t target,
                       const uint8_t *payload, uint32_t len)
{
    ...
    QWRT_UNUSED(payload); QWRT_UNUSED(len);
    return -1;
#endif
}
...
int qwrt_ctl_deliver_receipt(qwrt_t *rt, const uint8_t *payload, uint32_t len)
{
    ...
    if (sink) {
        qwrt_ctl_conn_write(sink, buf, len);          /* 端点控制器 */
    } else if (dir >= 0) {
...
int qwrt_control_endpoint_cmd(qwrt_t *rt, const char *bytes, size_t len,
                              void *sink)
{
    if (!rt || rt->magic != QWRT_MAGIC || !bytes) return -1;
    if (rt->config.control_plane != QWRT_CONTROL_LOCAL) return -1;
    ...
    if (target == local) {
        ...
        if (rc != 0) {
            ...
            if (n > 0 && (size_t)n < sizeof err)
                qwrt_ctl_conn_write(sink, err, (size_t)n);
        }
    }
    free(correl);
    return rc;
...
int qwrt_ctl_endpoint_init(qwrt_t *rt) { (void)rt; return -1; }
void qwrt_ctl_endpoint_close(qwrt_t *rt) { (void)rt; }
int qwrt_ctl_endpoint_owns(qwrt_t *rt, void *h) { (void)rt; (void)h; return 0; }
