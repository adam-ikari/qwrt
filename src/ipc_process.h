 ...
 * Parent spawns child via fork+exec of amoib-rt; child inherits one end of
...
#ifndef AM_IPC_PROCESS_H
#define AM_IPC_PROCESS_H
 ...
 * below can use am_proc_t without pulling in uv types (review: the typedef
 * used to sit mid-declaration list). Guarded: am_internal.h forward-declares
...
#define AM_IPC_PROTO_VERSION    1
#define AM_IPC_ROLE_WORKER      0
#define AM_IPC_ROLE_MAIN        1   /* M-P2：主RT 进程（宿主↔主RT 通道） */

/* 主RT 通道的本地标签（§4.3 逐跳相对寻址）：宿主=0，主RT=1。 */
#define AM_IPC_HOST_ID          0
#define AM_IPC_MAIN_ID          1
#define AM_IPC_HANDSHAKE_TIMEOUT_MS  5000
#define AM_IPC_TERMINATE_TIMEOUT_MS  2000
#define AM_IPC_READ_BUF_SIZE    65536

/* 子进程通道 fd 固定值（spawn 分层化）：调用方拼 argv 时 --parent-fd 一律
 * 写 3；am_proc_spawn 内部 socketpair 后，fork 出的 child 把子端
 ...
 * payload = 带 "amoib" 标记的 JSON（cJSON 解析，与握手同裁决：不手写）：
 *   {"amoib":1,"ready":V}      V=1 初始化就绪 / 0 失败（宿主据此决定 am_create）
 *   {"amoib":1,"idle":V}       V=0 宿主请求 idle / 1 主RT 已排空并判 idle（ack）
 *   {"amoib":1,"shutdown":1}   优雅关停（与 M-P1 的 {"cmd":"shutdown"} 并存）
 * 无 "amoib" 标记的 CONTROL payload 归上层（控制面消息/应用），classify 返回 NONE。 */
#define AM_IPC_CTL_READY_OK   "{\"amoib\":1,\"ready\":1}"
#define AM_IPC_CTL_READY_ERR  "{\"amoib\":1,\"ready\":0}"
#define AM_IPC_CTL_IDLE_REQ   "{\"amoib\":1,\"idle\":0}"
#define AM_IPC_CTL_IDLE_ACK   "{\"amoib\":1,\"idle\":1}"
#define AM_IPC_CTL_SHUTDOWN_MSG "{\"amoib\":1,\"shutdown\":1}"
#define AM_IPC_CTL_CLOSING      "{\"amoib\":1,\"closing\":1}"   /* M-P4 §9.3：
                                         * worker 自 close 前通知父（EOF 不再
                                         * 当作崩溃触发 onerror） */
#define AM_IPC_CTL_PING_MSG   "{\"amoib\":1,\"ping\":1}"   /* liveness 探测：
                                         * 宿主→对端；对端 C 层读泵就地直回。
                                         * 跨层形态加 "tp":[u16 链]（§8.2
                                         * root-relative 槽位链，宿主→任意
                                         * worker）：中间节点按链下投（读泵
                                         * 转发，不消费），目标读泵直回。 */
#define AM_IPC_CTL_PONG_MSG   "{\"amoib\":1,\"pong\":1}"   /* liveness 应答：
                                         * 对端读泵回显 corr（= ping seq）。
                                         * 跨层 PONG 回显 "tp"（过境标记：
                                         * 中间节点见 tp 沿父通道上行转发，
                                         * 不吃进自身 ping_seq 配对槽）。 */
#define AM_IPC_CTL_PFAIL_FMT  "{\"amoib\":1,\"pfail\":1,\"corr\":%d}"  /* 跨层
                                         * ping 转发失败（无对应子槽位/通道
                                         * 死）：中间节点回 corr=seq，宿主
                                         * 快速 -1（不白等 timeout）。 */
typedef enum {
    AM_IPC_CTL_NONE = 0,   /* 非 M-P2 协议 CONTROL payload */
    AM_IPC_CTL_READY,
    AM_IPC_CTL_IDLE,
    AM_IPC_CTL_SHUTDOWN,
    AM_IPC_CTL_SYSTEM,     /* 带 "amoib" 标记的其他系统消息（M-P4 closing 等）：
                              * 通道级，不进控制面命令路由器（CTL-1） */
    AM_IPC_CTL_PING,       /* liveness 探测（payload {"amoib":1,"ping":1}，
                              * 跨层加 "tp"） */
    AM_IPC_CTL_PONG,       /* liveness 应答（payload {"amoib":1,"pong":1}，
                              * 跨层回显 "tp"） */
    AM_IPC_CTL_PFAIL,      /* 跨层 ping 转发失败（{"amoib":1,"pfail":1,
                              * "corr":seq}） */
} am_ipc_ctl_kind_t;
...
/* Channel handle — see typedef above (am_proc_t), defined at the top of
 ...
 * Mock test builds (AM_USE_MOCK_LIBUV) only see the opaque typedef. */
...
/* 读泵回调（am_proc_start_read_cb）：cb(user, kind, source, corr, payload, len)。
...
typedef void (*am_proc_msg_cb_t)(void *user, int8_t kind, int32_t source,
                                   int32_t corr,
...
struct am_proc_s {
    ...
    int       role;           /* AM_IPC_ROLE_* */
    int       state;          /* am_proc_state_t */
    void     *parent_rt;      /* parent am_t* (for callbacks) */
    /* CTL-1：>0 时本通道上到达的命令类 CONTROL 信封交 am_control_route
     * 树路由（值为本节点槽位 id）；0 = 不路由（宿主侧读泵交 msg_cb）。 */
    int32_t   ctl_route_id;
    /* Read accumulator: frames = [4-byte LE len][envelope]; async reads via
     * am_proc_start_read / am_proc_start_read_cb. */
    uint8_t  *rbuf;
    size_t    rbuf_cap;
    size_t    rbuf_len;
    uint32_t  frame_len;      /* 0 = need 4-byte header */
    int       pipe_inited;    /* uv_pipe_init done (close via uv_close) */
    am_tx_t tx;             /* outbound spill buffer + flush timer */
...
typedef enum {
    AM_PROC_BUILD = 0,   /* pipe opened, awaiting handshake */
    AM_PROC_RUN   = 1,   /* handshake done, application messages flow */
    AM_PROC_DEAD  = 2    /* peer EOF or killed */
} am_proc_state_t;
 ...
 * AM_IPC_CHANNEL_FD (3), so caller-built argv references --parent-fd 3.
 *
 * exe:  任意可执行文件路径；NULL/空串 → auto-detect amoib-rt (AM_RT_SERVER
 *       env / /proc/self/exe dir + "amoib-rt" / AM_RT_PATH).
 * argv: 调用方拼好的 argv（argv[0] = 程序名），含 --parent-fd 3 等子进程
 *       参数；执行的是 exe 路径（argv[0] 只是显示名）。
 * role/id:  握手预期（child 发 {"v":V,"role":role,"id":id}）；role<0 →
 *           跳过握手。
 * require_handshake: 0 → spawn 后直接 AM_PROC_RUN，不做握手（用于任意
 *           可执行文件，child 不 speak amoib 信封协议）。
 * Returns 0 on success, am_err_t (<0) on failure. */
...
/* JS-managed delivery mode (spawn 分层化): 同 am_proc_start_read，但信封
...
void am_proc_free(am_proc_t *proc);   /* destroy + free struct */
/* ── Liveness ping（worker 进程→sub worker，镜像 rt_host.c 的 am_ping）──
 * 发 CONTROL{"amoib":1,"ping":seq}（corr = seq）并阻塞等待 sub worker C 层
 ...
 * The child process (amoib-rt) registers its inherited --parent-fd here so
 * bridge.c's js_pal_worker_emit can send envelopes upstream without a
 * am_proc_t (parent-side handle). Single channel per process. */
 ...
 * 父死亡 → -1。corr 语义同 am_ipc_child_emit。 */
 ...
 * 子进程管道读状态 g_rx / g_server_mode），libamoib 侧经函数指针注册访问——
 * 与 am_ipc_child_set_channel 同构：CLI/宿主进程不注册，pal.storageSync
 * 求值报错（worker 进程之外不可达）。
 *
 ...
 * 走孤儿自杀路径并返回 -1。 */
typedef int (*am_ipc_storage_sync_fn)(
    am_t *rt, const uint8_t *payload, uint32_t payload_len,
