    ...
    function fetch(input, init) {
      return new Promise(function(resolve, reject) {
        ...
        if (typeof pal2.httpRequestStream !== "function") {
          pal2.httpRequest(request.url, request.method, headersJson, bodyStr, function(status, data) {
            if (aborted) return;
    ...
    TextDecoder2.prototype.decode = function decode(input, options) {
      ...
      while (i < bytes.length) {
        ...
        if (byte < 128) {
          ...
          var codePoint = (byte & 7) << 18 | (bytes[i++] & 63) << 12 | (bytes[i++] & 63) << 6 | bytes[i++] & 63;
  ...
  function setupStreams(pal2) {
    ...
    class ReadableStream {
      ...
      tee() {
        ...
        var reading = false;
        var buffer = [];  // buffer chunks until both branches are ready
        var teeLog = (typeof console !== 'undefined' && console.log) ? function(m) { console.log('[tee] ' + m); } : function(){};

        teeLog('tee() called on source');

        function dispatchBuffered() {
          // flush buffer to any ready branches
          while (buffer.length > 0) {
            var chunk = buffer[0];
            var delivered = 0;
            if (!branch1Closed && branch1Controller) { branch1Controller.enqueue(chunk); delivered++; }
            if (!branch2Closed && branch2Controller) { branch2Controller.enqueue(chunk); delivered++; }
            if (delivered === 0) break;  // neither branch ready yet
            buffer.shift();
            teeLog('dispatched chunk to ' + delivered + ' branches, buffer remaining: ' + buffer.length);
          }
          if (buffer.length === 0 && reading) {
            // Source is done (reading=true and empty buffer), close branches
            teeLog('closing branches (source done, buffer empty)');
            if (!branch1Closed && branch1Controller) { branch1Controller.close(); branch1Closed = true; }
            if (!branch2Closed && branch2Controller) { branch2Controller.close(); branch2Closed = true; }
          }
        }
        ...
        function pullAll() {
          if (reading) return;
          reading = true;
          teeLog('pullAll: starting to read from source');
          var reader = source.getReader();
          function readNext() {
            reader.read().then(function(result) {
              if (result.done) {
                teeLog('pullAll: source done, flushing buffer (' + buffer.length + ' items)');
                dispatchBuffered();
                if (buffer.length === 0) {
                  if (!branch1Closed && branch1Controller) { branch1Controller.close(); branch1Closed = true; }
                  if (!branch2Closed && branch2Controller) { branch2Controller.close(); branch2Closed = true; }
              ...
              teeLog('pullAll: got chunk from source');
              buffer.push(result.value);
              dispatchBuffered();
              readNext();
            }).catch(function(e) {
              teeLog('pullAll: error: ' + (e.message || e));
        ...
        function createBranch() {
          teeLog('createBranch: creating branch');
          return new ReadableStream({
            start: function(controller) {
              teeLog('branch start called');
              // QuickJS polyfill doesn't invoke pull(), eagerly start reading
              pullAll();
              // Dispatch any already-buffered data to this branch
              dispatchBuffered();
            },
            cancel: function() {
              teeLog('branch cancel called');
            }
          });
        }
        ...
        teeLog('both branches created, controllers set');
        // After both controllers are set, flush buffer to both
        dispatchBuffered();
        teeLog('tee() returning [branch1, branch2]');
        ...
        self._writable = new WritableStream({
          ...
          close: function() {
            ...
            if (typeof pal2.nativeCompress !== "function") {
              self._readable._controller.error(new TypeError("Compression extension not available"));
              return Promise.resolve();
            }
            try {
