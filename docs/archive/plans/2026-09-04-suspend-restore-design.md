...
> 范围：amoib 运行时（QuickJS-ng 嵌入式）的 context 软挂起/恢复——修复 G1（suspend 不释放内存）/G2（destroy 带 pending job → UAF），激活 G3（扩展 suspend/resume 钩子死代码），明确设计边界（G4 非枚举属性、G5 运行时状态不可序列化），与多进程模型 §14 正交。
> 引用源码：`src/context.c`、`polyfill/src/context.js`、`src/extension.c`、`src/am_internal.h`、`src/amoib.c`（teardown）、`src/thread.c`（主循环/微任务 flush）、`deps/quickjs-ng/quickjs.{c,h}`（job 队列 API）、`test/test_suspend_gtest.cpp`（现有测试）。

---

## TL;DR（决策摘要）

1. **G2 修复（UAF，必修）**：在 `am_ctx_destroy` 中、`JS_FreeContext` 之前，调用 QuickJS-ng 新增公开 API `JS_DrainPendingJobsForContext(rt->jsrt, ctx->jsctx)` 遍历 `rt->job_list` 删除 `e->ctx == ctx` 的 pending job。补丁走 `deps/quickjs-ng-*.patch` 机制（同 C99 atomics patch），CMake 无条件应用。
2. **G1 修复（内存释放）**：`am_ctx_serialize` 成功写盘后调 `am_ctx_destroy` 释放 JSContext，设 `rt->contexts[ctx_id] = NULL`（槽位空出）；恢复时 `am_ctx_rebuild` 已有 `if (rt->contexts[ctx_id]) am_ctx_destroy(...)` 兜底，复用 `am_ctx_create_at` 重建。`ctx->suspended` 标记不再需要（destroy 后 ctx 结构已 free，槽位 NULL 即挂起态）。
3. **G3 修复（扩展钩子激活）**：`am_ctx_serialize` 写盘前调 `am_ext_suspend_all`，`am_ctx_rebuild` restore 后调 `am_ext_resume_all`。当前所有扩展的 suspend/resume 是 no-op，接线零行为变更，为未来扩展状态保存铺路。
...
**挂起**（`am_ctx_serialize`，context.c:366–393）：
1. 调目标 ctx 的 `__am_ctx_capture__`（JS 侧遍历 `Object.keys(globalThis)`，排除 `_pristine`/`_infra`，结构化克隆可枚举全局属性）。
2. 取 ArrayBuffer 字节，同步写盘（`am_write_file`）。
3. **不销毁 JSContext**——ctx 留在 `rt->contexts[]`，内存不释放。

**恢复**（`am_ctx_rebuild`，context.c:395–435）：
1. 若槽位有残留 ctx → `am_ctx_destroy`（兜底）。
2. `am_ctx_create_at` 重建 JSContext + 重注入 polyfill + 初始化扩展。
3. eval `script_ref`（init 脚本）。
4. 读盘 → `__am_ctx_restore__` 写回 globalThis。

### 1.2 缺陷

| 缺陷 | 严重度 | 现状 | 影响 |
|------|--------|------|------|
| **G1** | 中 | `am_ctx_serialize` 只写文件不销毁 ctx | suspend 后内存不释放；注释说"销毁"实不销毁 |
...
| **G3** | 低 | `am_ext_suspend_all`/`resume_all` 定义但零调用方，全 no-op | 扩展状态不保存/恢复（当前无实际后果） |
...
**复现路径**：
1. `amContext.spawn` 子 context。
2. 子 context eval 创建 Promise（`Promise.resolve().then(...)`），但 `.then` 回调未执行。
3. `amContext.suspend(1, path)` 或 `amContext.destroy(1)`。
4. `am_flush_microtasks`（thread.c:59，每轮 `uv_run` 后）→ `JS_ExecutePendingJob` → 访问已释放的 `e->ctx` → **UAF/crash**。
...
**`am_ctx_destroy`**（context.c:249–280），在 `JS_FreeContext` **之前**插入 drain：
...
void am_ctx_destroy(am_t *rt, am_ctx_t *ctx)
{
     ...
     * the next JS_ExecutePendingJob (am_flush_microtasks) triggers UAF. */
...
**为什么 drain 在 `am_ext_destroy_all` + `cleanup_resources` 之后**：扩展 destroy hook 和 cleanup 可能排入新的 pending job（如 timer cancel 回调），drain 必须在所有可能入队操作之后。

**`am_thread_teardown`（amoib.c:260–280）已有全量 drain**：步骤 2 的 `while (JS_ExecutePendingJob > 0)` 在 runtime 关闭时清空所有 job。G2 修复不影响 teardown——teardown 先 join workers、drain queue、abort stream、然后逐 ctx destroy（此时 drain per-ctx），再 free runtime。per-ctx drain + runtime-level drain 不冲突（前者删该 ctx 的，后者删剩余的）。

### 2.4 G2 修复验证

新增测试（test_suspend_gtest.cpp）：
- **`destroy_with_pending_job_no_uaf`**：spawn 子 ctx → eval `Promise.resolve().then(...)` 但不 flush 微任务 → `amContext.destroy(1)` → 主 context `am_flush_microtasks`（通过 eval 触发一轮 uv_run）→ 不崩溃 + 主 ctx 正常。
...
**`am_ctx_serialize`**（context.c:366–393）成功写盘后调 `am_ctx_destroy`：
...
int am_ctx_serialize(am_t *rt, int ctx_id, const char *state_path)
{
    ...
    /* G1: 写盘成功后销毁 JSContext，释放内存。槽位由 am_ctx_destroy
     * 置 NULL，后续 am_ctx_rebuild 在该槽位重建。 */
...
- `am_ctx_rebuild` 已有 `if (rt->contexts[ctx_id]) am_ctx_destroy(...)` 兜底（context.c:402–404），对空槽位是 no-op，无需改动。
- `am_get_ctx_by_id` 对空槽返回 NULL → 后续操作自然报错（`AM_ERR_NOT_FOUND`）。

### 3.2 `ctx->suspended` 字段处理

`am_internal.h:153` 的 `int suspended` 字段在 G1 修复后**不再使用**（destroy 后 ctx 结构已 free，槽位 NULL 即"挂起"态）。

**决定**：保留字段不删（ABI 兼容，结构体仍 calloc 初始化为 0），但在 `am_ctx_create_at` 中注释移除"1 if suspended"含义。`am_ctx_create_at` 已经 `ctx->suspended = 0`（context.c:118），无变更需要。
...
**新增**：`suspend_releases_memory`（ASan 构建）：suspend 后 `am_get_ctx_by_id(rt, id)` 返回 NULL（槽位空），主 ctx 正常。这验证 G1 的核心不变量。
...
`am_ext_suspend_all`/`am_ext_resume_all`（extension.c:66–101）已实现，遍历 `ctx->extensions[]` 调各扩展的 `suspend`/`resume` 函数指针。但：
- **零调用方**：grep 全库无调用点。
...
1. **`am_ctx_serialize`**：在 capture **之前**调 `am_ext_suspend_all(rt, cctx)`——让扩展在 ctx 还活着时保存状态（如 crypto DRBG 种子、wasm 引用计数）到自身存储（per-runtime 或 per-extension struct）。

2. **`am_ctx_rebuild`**：在 `__am_ctx_restore__` **之后**调 `am_ext_resume_all(rt, cctx)`——让扩展从保存的状态恢复。

```c
// am_ctx_serialize, before capture:
am_ext_suspend_all(rt, cctx);   // G3: 让扩展保存状态（当前全 no-op）
// ... existing capture + write ...
am_ctx_destroy(rt, cctx);      // G1: 释放

// am_ctx_rebuild, after restore:
// ... existing create_at + eval + restore_bytes ...
am_ext_resume_all(rt, cctx);   // G3: 让扩展恢复状态（当前全 no-op）
```

### 4.3 为什么当前 no-op 也接线

1. **消除死代码路径**：`am_ext_suspend_all`/`resume_all` 从"定义但零调用"变为"调用但 no-op"——未来扩展按需实现真 suspend/resume 时，调用点已就位。
2. **零行为变更**：所有现有扩展的 suspend/resume 都是 `return 0`，接线不改变任何运行时行为。
3. **零测试影响**：现有测试不涉及扩展状态，接线后测试不变。

### 4.4 扩展状态归属

当前扩展状态是 **per-JSRuntime** 而非 per-JSContext（如 `rt->ec_drbg`，am_internal.h:281–288）。G1 修复只销毁 JSContext（JSRuntime 存活），所以 per-runtime 扩展状态在 suspend/restore 期间天然存活。per-context 的扩展注册（JSClass、global API）由 `am_ctx_create_at` 的 `am_ext_init_all` 重建——这是现有行为，G3 接线不改变它。
...
`__am_ctx_capture__` 用 `Object.keys(globalThis)` 只捕获**可枚举字符串键**。以下不被捕获：
- `Object.defineProperty(globalThis, 'x', {enumerable: false})` 定义的非枚举属性。
- `Symbol` key 属性。
- getter 抛异常的属性（`try { v = globalThis[n] } catch { continue }`）。

**决定**：不扩展为 `Object.getOwnPropertyNames`（含非枚举）。polyfill 基础设施由 `am_ctx_create_at` 重新注入（`_pristine` 排除 + rebuild 重 eval），用户非枚举全局是罕见模式，YAGNI。在 `context.js` 注释中标注此限制。
...
**G2 修复使此边界更安全**：suspend（含 G1 destroy）前，`am_ctx_destroy` 的 `am_ctx_cleanup_resources` 取消该 ctx 的所有定时器；`JS_DrainPendingJobsForContext` 清除该 ctx 的 pending job。但活跃 I/O（per-runtime）不在 context 级别处理范围——这与多进程模型 §14.2 正交铁则一致（context 挂起不波及 worker/I/O，I/O 归 runtime 生命周期）。
...
**不变量**：
- `am_ctx_serialize` 成功后 `rt->contexts[id] == NULL`（G1）。
- `am_ctx_rebuild` 对 empty 槽位 = `create_at` + eval + restore；对 occupied 槽位 = 先 destroy 再重建（已有兜底）。
- `am_ctx_destroy` 安全地处理有 pending job 的 ctx（G2 drain）。
- 活跃 ctx 不可 suspend/destroy（`ctx_id == rt->active_ctx_id` 返回 `AM_ERR_BUSY`，context.c:371/440）。
...
### 6.3 与现有 `am_ctx_serialize`/`rebuild`/`destroy` 的差异

| 函数 | 现状 | 修复后 |
|------|------|--------|
| `am_ctx_serialize` | capture → write（不 destroy） | `ext_suspend` → capture → write → **destroy**（G1+G3） |
| `am_ctx_rebuild` | destroy 兜底 → create_at → eval → restore | + **`ext_resume`** after restore（G3） |
| `am_ctx_destroy` | ext_destroy → cleanup → free_ctx | + **drain pending jobs** before free_ctx（G2） |
...
> context.c：`am_ctx_spawn/suspend/resume/serialize/rebuild`（复用，零新码）
...
若 `worker_backend=PROCESS`（M-P1 后），主 RT 进程的 `am_thread_teardown` 步骤 2（全量 `JS_ExecutePendingJob` drain）和步骤 4（per-ctx `am_ctx_destroy`）仍按序执行。G2 的 per-ctx drain 在步骤 4 内、每个 `JS_freeContext` 前调用——单进程和多进程下同等必要。
...
| `suspend_releases_slot` | G1 | suspend 后 `am_get_ctx_by_id` 返回 NULL（槽位空出）；resume 回同 id 成功 |
| `suspend_then_double_suspend` | G1 | suspend（destroy）→ 再 suspend 同 id → `AM_ERR_NOT_FOUND`（槽位已空） |
| `ext_hooks_called_on_suspend_resume` | G3 | （需扩展支持或 mock）验证 `am_ext_suspend_all`/`resume_all` 被调用 |

### 8.3 测试实现要点

- **UAF 测试**（G2）：关键是在 destroy/suspend 后**不手动 flush**，而是让主循环的 `am_flush_microtasks`（thread.c:59）自然触发。测试通过 `host_eval` 在主 ctx 触发一轮 `uv_run`（eval 本身不触发微任务 flush——需在子 ctx eval Promise.then 后立即 destroy/suspend，然后 eval 主 ctx 代码触发 uv_run 一轮）。
- **`suspend_releases_slot`**：直接检查 `am_get_ctx_by_id(rt, 1) == NULL`——需通过 C 层断言或 JS 侧间接验证（suspend 后 `amContext.suspend(1, ...)` 再调应抛 NOT_FOUND，因为槽位已空）。
- **`ext_hooks_called`**：当前扩展全 no-op，无法直接验证调用。方案：(a) 在测试 build 中定义一个 test-only 扩展，其 suspend/resume 设 flag；或 (b) 验证 `am_ext_suspend_all`/`resume_all` 的调用路径（C 单元测试直接调）。倾向 (b)——C 单元测试更直接。
...
3. **`am_ctx_destroy` 调用**：在 `JS_freeContext` 前调 `JS_DrainPendingJobsForContext`。
4. **测试**：`destroy_with_pending_job_no_uaf` + `suspend_with_pending_job_no_uaf`。

### 阶段 2：G1 修复（内存释放）

1. **`am_ctx_serialize`**：写盘成功后调 `am_ctx_destroy`。
2. **验证**：现有 `child_state_roundtrip`/`resume_into_fresh_slot` 不退化 + 新增 `suspend_releases_slot`/`suspend_then_double_suspend`。

### 阶段 3：G3 修复（扩展钩子激活）

1. **`am_ctx_serialize`**：capture 前调 `am_ext_suspend_all`。
2. **`am_ctx_rebuild`**：restore 后调 `am_ext_resume_all`。
...
| 4 | 异步状态前置检查 | **不强制**（文档标注 G5 边界） | G2 drain 已消除 UAF；强制 `AM_ERR_BUSY` 会破坏现有 suspend 立即调用的用法；用户应在 drain async 后 suspend |
...
| 断言 | 证据位置 |
|------|----------|
| `am_ctx_serialize` 不销毁 ctx（G1 现状） | `src/context.c:366–393`（只写文件，无 `am_ctx_destroy` 调用） |
| `ctx->suspended` 从未设 1 | `src/context.c:118`（仅 init 0）；grep `suspended` 全库仅此一处赋值 |
| `am_ctx_rebuild` 有 destroy 兜底 | `src/context.c:402–404` |
...
| `am_flush_microtasks` 每轮执行 job | `src/thread.c:59–60`（`JS_IsJobPending` busy 判定） |
| `am_thread_teardown` 全量 drain | `src/amoib.c:263–267`（`while JS_ExecutePendingJob > 0`） |
| 扩展 suspend/resume 无调用方（G3） | grep `am_ext_suspend_all`/`resume_all` → 仅定义+声明，零调用 |
| 扩展 suspend/resume 全 no-op | `src/ext_crypto.c:1345`、`ext_wasm3.c:1970`、`ext_textcodec.c:292`、`ext_wamr.c:1821`、`ext_web_wasm.c:84` |
| `am_ext_suspend_all` 签名 | `src/am_internal.h:392`：`int am_ext_suspend_all(am_t *rt, am_ctx_t *ctx)` |
...
| `AM_MAX_CONTEXTS` | `src/am_internal.h:29`（64） |
