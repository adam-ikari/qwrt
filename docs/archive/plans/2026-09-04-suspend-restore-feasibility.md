...
> 日期：2026-09-04
> 范围：amoib 运行时（QuickJS-ng 嵌入式）的 context 挂起/恢复（suspend/resume）与运行时级快照（snapshot/checkpoint）能力。评估从「当前 Task 5 软挂起」到「完整运行时快照」之间的可行性光谱，识别引擎硬限制、现状缺陷、以及与多进程模型（§14）的组合关系。
...
3. **扩展 suspend/resume 钩子是死代码**：`am_ext_t.suspend/resume` 已定义、全 no-op、无任何调用方——挂起/恢复不触发扩展状态保存/恢复。
...
- `__am_ctx_capture__`：遍历 `Object.keys(globalThis)`，排除 `_pristine`（polyfill setup 完成后、本模块挂载前的全局键快照）和 `_infra`（`__am_ctx_capture__`/`__am_ctx_restore__`/`amContext`），对每个剩余键用 `__am_serialize__`（structured-clone 内核）序列化，不可克隆的记入 `skipped` 数组，返回 `{props, skipped}` 的字节。
- `__am_ctx_restore__`：反序列化字节 → 遍历 `props`，用 `Object.defineProperty` 写回 `globalThis`（防 `__proto__` 污染），返回 `skipped` 列表。
- `amContext.spawn/suspend/resume/destroy`：经 `pal.contextSpawn/contextSuspend/contextResume/contextDestroy` 驱动 C 层。

**C 侧**（`src/context.c` + `src/bridge.c`）：

- `am_ctx_spawn`（context.c:343）：创建新 context（`am_ctx_create` 找空槽），eval init script。
- `am_ctx_serialize`（context.c:366）：调用目标 ctx 的 `__am_ctx_capture__` → 取 ArrayBuffer → **同步写盘**（`am_write_file`，fopen/fwrite/fclose）。**不销毁 JSContext。**
- `am_ctx_rebuild`（context.c:395）：若槽位有残留 ctx → `am_ctx_destroy` → `am_ctx_create_at`（新 JSContext + 重新注入 polyfill + 初始化扩展）→ eval `script_ref` → 读盘 → `__am_ctx_restore__` 写回 globalThis。
- `am_ctx_destroy`（context.c:249）：`am_ext_destroy_all` → `am_ctx_cleanup_resources`（取消定时器）→ `JS_FreeContext` → 从 `rt->contexts[]` 摘除 → `free(ctx)`。

**桥接**（`bridge.c:1675-1743`）：`pal.contextSuspend` 调 `am_ctx_serialize`；`pal.contextResume` 调 `am_ctx_rebuild`；`pal.contextDestroy` 调 `am_ctx_destroy_id`。

### 1.2 序列化内核覆盖面（`__am_serialize__` / structured-clone.js）

`__am_serialize__` 是 Worker postMessage 共用的序列化内核，支持：
...
| Map / Set | ✅ | |
| Date / RegExp / BigInt | ✅ | |
| MessagePort 引用 | ✅ | `__am_port_from_ref__` 重建 |
| **函数** | ❌ | DataCloneError |
...
**后果**：在 runtime 仍在运行时 `JS_FreeContext(ctx)` 一个有 pending job 的 context → `rt->job_list` 中残留 `e->ctx = 已释放的 ctx` → 下次 `JS_ExecutePendingJob(rt->jsrt, &job_ctx)`（在 `am_flush_microtasks` 中每轮主循环调用）→ **UAF（use-after-free）**。

`am_thread_teardown`（amoib.c:263-267）在销毁 contexts **之前**排空所有 pending job，所以 **runtime 整体销毁路径安全**。但 **单 context destroy（suspend/destroy 路径）不在 teardown 上下文中**，没有这个保护。

### 2.3 ctx->suspended 字段

`am_internal.h:153` 定义 `int suspended`，在 `am_ctx_create_at` 中初始化为 0（context.c:118），**全代码库无任何位置将其设为 1**。死字段。
...
**现状**：`context.js` 注释说"挂起 = ... 然后 C 销毁该 JSContext"；`bridge.c:1691` 注释说"序列化全局克隆字节写盘 + 销毁 ctx"。但 `am_ctx_serialize`（context.c:366-393）**只写文件，不调 `am_ctx_destroy`**。JSContext 留在 `rt->contexts[]` 中，内存不释放。

**影响**：suspend 后内存占用不减。如果"挂起"的语义预期是"暂停并释放资源"（类比浏览器 tab discard / Service Worker eviction），当前实现不满足。用户须显式调 `amContext.destroy(id)` 才释放。

**严重度**：中（功能正确但语义/文档不符；内存不释放是资源浪费，非崩溃）。

### G2：销毁带 pending job 的 context → UAF

**现状**：`am_ctx_destroy` → `JS_FreeContext` 不清理 `rt->job_list` 中属于该 ctx 的 pending job。若被销毁的 ctx 有 pending Promise 反应（`.then` callback 未执行），`rt->job_list` 中残留 `e->ctx = freed_ptr`。下次 `am_flush_microtasks`（thread.c:21-27，每轮 `uv_run` 后调用）执行 `JS_ExecutePendingJob` → **UAF**。
...
4. `am_flush_microtasks` → crash 或内存损坏。

**缓解**：`am_ctx_rebuild` 在 destroy 旧 ctx 前没有 drain pending jobs。`am_ctx_cleanup_resources` 只取消定时器，不排空 job 队列。

**严重度**：高（UAF，安全/稳定性问题）。

### G3：扩展 suspend/resume 钩子是死代码

**现状**：`am_ext_t.suspend`/`resume` 函数指针已定义（amoib.h:66-67），`am_ext_suspend_all`/`am_ext_resume_all` 已实现（extension.c:66-101），但：

- **无任何调用方**：grep `am_ext_suspend_all`/`am_ext_resume_all` 在 src/ 中只有定义和声明，零调用点。
...
**影响**：挂起/恢复不保存/恢复扩展状态（crypto DRBG 种子、wasm 实例、codec 状态）。但鉴于当前扩展状态都是 per-JSRuntime（`rt->ec_drbg` 等，见 am_internal.h:281-288）而非 per-context，且挂起不销毁 JSRuntime，这个缺口目前无实际后果——但若未来 G1 修复为真销毁 JSContext，扩展的 per-JSRuntime 状态仍会存活（JSRuntime 不销毁），只是 per-JSContext 的扩展注册（JSClass、global API）需重建（`am_ctx_create_at` 的 `am_ext_init_all` 已做）。

**严重度**：低（当前无实际影响；是架构预留但未接线）。

### G4：非枚举全局属性丢失

`__am_ctx_capture__` 用 `Object.keys(globalThis)` 只捕获**可枚举字符串键**属性（context.js:30）。三类全局状态被漏：`Object.defineProperty(..., {enumerable: false})` 定义的属性、Symbol-keyed 属性、getter 抛异常的属性（`try { v = globalThis[n] } catch { continue }`，context.js:35）。

**影响**：恢复后这些属性缺失。polyfill 注入的 API（含 `localStorage`，它是 `enumerable: true` 但在 `_pristine` 快照之前挂载，见 G7）由 `_pristine` 排除、恢复时由 `am_ctx_create_at` 重新注入，所以不丢。丢的是用户代码自己用非枚举/Symbol 定义的全局。
...
**定时器**：`ctx->handles[]` / `ctx->timer_resolves[]` / `ctx->timer_cbds[]`（am_internal.h:155-158）是 C 层 libuv timer + JSValue resolve/reject callback。挂起不取消定时器（`am_ctx_serialize` 不调 `am_ctx_cleanup_resources`），但如果 G1 修复为销毁 ctx，则 `am_ctx_destroy` 调 `am_ctx_cleanup_resources` 取消定时器——定时器状态丢失。

**活跃 I/O**：libuv handle（TCP listener、fetch stream、timer）在 `rt->loop` 上，per-runtime 而非 per-context。`rt->active_stream`（am_internal.h:223）是 per-runtime 的活跃 HTTP op 指针。Context 挂起不影响 runtime 的 I/O。
...
1. **Web `localStorage`**（`polyfill/src/local-storage.js`）：数据在 JS 闭包 `map` + 磁盘 JSON 文件（`pal.localStoragePath()`，每次 `setItem`/`removeItem`/`clear` 经 `pal.fsWriteSync` 原子写 temp+rename）。`globalThis.localStorage` 是 `enumerable: true`（local-storage.js:147），但挂载发生在**所有 polyfill setup 之后、`context.js` 的 `_pristine` 快照之前**（context.js:15-18）→ 落在 `_pristine` 内 → 被 capture 排除。恢复时 `am_ctx_create_at` 重注入 polyfill → `setupLocalStorage` 重跑 → `load()` 从文件读回 → **数据天然存活**（真相在磁盘，不在快照）。
2. **`amoib.storage`（异步扩展 API）**：C 层 `rt->store`（`uv_io_store_entry_t *`，am_internal.h:218-220），per-runtime 而非 per-context，`am_thread_teardown` 步骤 6 释放。context 挂起/销毁完全不触碰它。

**结论**：context 挂起/恢复不需要保存 storage 状态——Web localStorage 靠文件持久化，`amoib.storage` 归 runtime 生命周期。这是正确设计，非缺口。多进程模型 §10.2 的单所有者代理（storage 归主 RT，操作经 `kind=STORAGE` 信封）与此正交：它解决的是**跨进程并发写同一文件**，不改变「storage 状态不进 context 快照」这一条。
...
> | context | context.c：`am_ctx_spawn/suspend/resume/serialize/rebuild`（复用，零新码） | 同一 JSRuntime 堆内多 JSContext；软隔离（可挂起/序列化/重建） |
...
**唯一交叉点**：若 G1 修复为真销毁 JSContext（释放内存），而 context 恰好在 `worker_backend=PROCESS` 的主 RT 上——destroy 前需 drain pending job（G2 修复），否则主 RT 的 `am_flush_microtasks` 会访问已释放的 ctx。这在单进程模型下同样成立（job_list 是 runtime 级），与进程模型无额外耦合。
...
**G2 修复**（UAF，必修）：在 `am_ctx_destroy` 中，`JS_FreeContext` **之前**排空属于该 ctx 的 pending job。两种方式：
...
- **方式 2（保守）**：在 `am_ctx_destroy` 前调 `JS_ExecutePendingJob(rt->jsrt, &job_ctx)` 循环**直到清空**（等同 `am_flush_microtasks`），然后 destroy。风险：执行其他 ctx 的 pending job（副作用不可控），但 QuickJS job 队列不区分 ctx。

**推荐方式 1**：新增 QuickJS-ng 补丁函数 `JS_DrainPendingJobsForContext(JSRuntime *rt, JSContext *ctx)`，遍历 `rt->job_list` 删除 `e->ctx == ctx` 的 entry（free argv + entry），返回删除数。`am_ctx_destroy` 在 `JS_FreeContext` 前调用。与 `deps/quickjs-ng` 现有补丁机制一致（项目已对 quickjs-ng 打补丁）。

**G1 修复**（内存释放，可选）：`am_ctx_serialize` 成功写盘后调 `am_ctx_destroy` 释放 JSContext，设 `ctx->suspended = 1` 标记槽位已挂起。`am_ctx_rebuild` 已有 `if (rt->contexts[ctx_id]) am_ctx_destroy(...)` 兜底，无需改动。`ctx->suspended` 字段已有但未使用，激活即可。

**影响面**：
- `context.js` 注释与实现对齐。
- `bridge.c` 注释与实现对齐。
- `test_suspend_gtest.cpp` 现有测试不变（`child_state_roundtrip` 不依赖 suspend 后 ctx 存活；`resume_into_fresh_slot` 已测 destroy → resume 路径）。
- `am_ctx_serialize` 后 `am_get_ctx_by_id` 返回 NULL（ctx 已销毁）——需确认 `pal.contextSuspend` 的 JS 侧不依赖 suspend 后 ctx 存活（context.js 的 `amContext.suspend` 只调 `pal.contextSuspend` 返回 undefined，不查 ctx）。

### 方案 B：增强软挂起（扩展状态 + 异步 drain）

在方案 A 基础上：

1. **激活扩展钩子**：`am_ctx_serialize` 前调 `am_ext_suspend_all`，`am_ctx_rebuild` 后调 `am_ext_resume_all`。当前所有扩展的 suspend/resume 是 no-op，但接线后扩展可按需实现状态保存/恢复（如 crypto DRBG 种子、wasm module 引用）。
2. **异步 drain 前置**：suspend 前要求 ctx 无 pending async work（类似 `am_wait_idle` 但 per-ctx）。`am_ctx_serialize` 检查 `JS_IsJobPending` 且 ctx 无活跃 timer → 否则返回 `AM_ERR_BUSY`。避免 G2 的 job 队列问题在源头消除。
...
3. **扩展钩子接线（方案 B）**：是否在 suspend/resume 路径调 `am_ext_suspend_all`/`am_ext_resume_all`。当前 no-op 但接线后为未来扩展状态保存铺路。**倾向接线**（零成本，消除死代码路径）。
4. **异步状态前置检查**：suspend 前是否强制要求 ctx idle（无 pending job / 无活跃 timer）。**倾向加检查**（返回 `AM_ERR_BUSY`），从源头消除 G2 并明确"挂起时机"契约。
...
| 断言 | 证据位置 |
|------|----------|
| suspend 不销毁 JSContext | `src/context.c:366-393`（`am_ctx_serialize` 只写文件，无 `am_ctx_destroy` 调用） |
| `ctx->suspended` 从未设 1 | `src/context.c:118`（仅 init 0）；grep `suspended` 全库仅此一处赋值 |
| `am_ctx_rebuild` 有 destroy 兜底 | `src/context.c:402-404`（`if (rt->contexts[ctx_id]) am_ctx_destroy(...)`） |
...
| am_thread_teardown 先 drain job | `src/amoib.c:263-267`（`while (JS_ExecutePendingJob > 0)`） |
| am_flush_microtasks 每轮执行 job | `src/thread.c:21-27` + `:60`（`JS_IsJobPending` busy 判定） |
| 扩展 suspend/resume 无调用方 | grep `am_ext_suspend_all`/`am_ext_resume_all` → 仅定义+声明，零调用 |
