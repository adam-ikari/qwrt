# amoib HTTPServer Extension — Design Spec

**Date:** 2026-08-15
**Status:** Draft for review
**Related:** uvhttp (/home/gem/project/uvhttp), amoib extension registry (am_ext_registry.h)

## 1. Overview & Goals

Provide the **full uvhttp capability set** to amoib (the QuickJS-ng embeddable
...
the same single-threaded libuv loop amoib already owns.
...
- **Build gate:** `AM_WITH_HTTPSERVER` CMake option (default **ON**). When OFF,
  `serve` is absent and zero uvhttp code is compiled in.

## 1.1 Feature mapping (uvhttp → amoib)

| uvhttp feature | uvhttp macro | amoib wiring |
|---|---|---|
| HTTP/1.1 core | always | uvhttp core sources (15 .c) |
| HTTPS/TLS | `UVHTTP_FEATURE_TLS=1` | mbedTLS — amoib's existing deps (3.6.6) |
...
| Compression | `UVHTTP_FEATURE_COMPRESSION=1` | miniz — amoib's existing deps |
...
- uvhttp's `uvhttp_server_new(uv_loop_t*, ...)` accepts an **external loop** — amoib
  passes `&rt->loop` (the loop owned by the amoib thread). All accept/read/write/
  handler events run on the amoib thread, preserving the project's
  single-threaded, lock-free philosophy: **no new threads, no locks**.
- uvhttp is compiled **from source into the amoib library** (not via uvhttp's own
  CMake, which uses ExternalProject to build a *second* libuv — that would
  collide with amoib's vendored libuv).
  ...
  - Deps (vendored inside uvhttp): `llhttp` (llhttp.c), `xxhash`, `cJSON`,
    `uthash` (header-only)
- **Third-party libs are unified with amoib's existing deps** (no duplicates):
  - **libuv** → amoib's vendored libuv (`uv_a`); uvhttp never builds its own.
  - **mbedTLS** → amoib's deps/mbedtls (3.6.6). uvhttp's TLS/WS code compiles
    against amoib's mbedtls headers; **verify API compatibility at build time**
    (risk item — uvhttp's own vendored mbedtls may differ in version; the
    extension build must pass uvhttp's compile tests).
  - **miniz** → amoib's deps/miniz (already used by AM_WITH_COMPRESS). uvhttp's
    compression code uses the miniz API (`mz_*` / `miniz_*`); unify include path.
  - llhttp/xxhash/cJSON/uthash have **no amoib counterpart** → compiled from
    uvhttp's deps/.
- CMake: `AM_WITH_HTTPSERVER` (option, default ON) + `UVHTTP_DIR` (default
  `${CMAKE_CURRENT_SOURCE_DIR}/../uvhttp`, overridable), wiring following the
  existing `AM_WITH_*` extension pattern.
...
- `include/amoib/ext_http_server.h` — `extern const am_ext_t am_http_server_ext;`
- `src/ext_http_server.c` — gated by `#if AM_WITH_HTTPSERVER`; registers
  `serve` (and its companion classes) via the extension init callback.
- Registry: `am_ext_registry.h` adds
  `AM_EXT_IF_WITH(HTTPSERVER, &am_http_server_ext)`.

### 2.3 Request/Response reuse

amoib's fetch polyfill already defines WHATWG `Headers` / `Request` / `Response`
...
- C-side: each live `uvhttp_ws_connection_t*` maps to a JS wrapper stored in
  the ws-handler `user_data` registry; callbacks invoked from uvhttp on the amoib
...
1. uvhttp parses the request on the amoib thread and calls our C handler
...
amoib main loop already flushes microtasks between `uv_run` iterations, so the
...
- Runtime teardown (`am_thread_teardown` → `am_ctx_destroy` → extension
...
```cmake
option(AM_WITH_HTTPSERVER "Build the uvhttp-based HTTP server extension" ON)
set(UVHTTP_DIR "${CMAKE_CURRENT_SOURCE_DIR}/../uvhttp" CACHE PATH "...")
```

When ON (all uvhttp features):
- Sources appended to the amoib library: `src/ext_http_server.c` + uvhttp core
...
- Include dirs added: `UVHTTP_DIR/include`, `UVHTTP_DIR/deps/llhttp/include`,
  ...
  `UVHTTP_DIR/deps/cjson` — plus amoib's mbedtls/miniz include dirs (already in
  the build when AM_WITH_TLS / AM_WITH_COMPRESS are ON).
- Defines: `AM_WITH_HTTPSERVER=1`, `UVHTTP_FEATURE_TLS=1`,
...
- Links: amoib's `uv_a` (libuv), `mbedtls`/`mbedcrypto`/`mbedx509` (amoib's TLS
  build), `miniz` (amoib's compress build).
- Registry: `AM_EXT_IF_WITH(HTTPSERVER, &am_http_server_ext)` +
  `AM_WITH_HTTPSERVER=$<BOOL:...>` in the PUBLIC definitions list.
...
- **CLI smoke test:** `amoib -e 'serve(...)'` + curl; listening handle keeps the
  loop busy so the CLI waits for `server.close()` or signal.
- **Feature-gate test:** build with `AM_WITH_HTTPSERVER=OFF` → `serve` is
