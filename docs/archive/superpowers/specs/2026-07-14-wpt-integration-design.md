...
amoib 是 WinterCG-compatible 的 QuickJS 运行时，但缺少上游规范的合规测试。现有 `test_compliance_gtest` 是手写的 ~50 个测试，不能穷尽、易遗漏边界。
...
Spike 验证（2026-07-14）：testharness.js + 注入的 `print` + shell report + 真实 URL `.any.js` 测试在 amoib（mock PAL）下完整运行，PASS/FAIL 结果正确捕获。
...
C 程序，用 amoib（mock PAL）逐个跑 `.any.js`：
1. 创建 amoib runtime（mock PAL，`config.host_data` 存 runner state）
...
5. eval 测试文件（`.any.js`）
6. `am_tick`（drain async，对 sync test 是 no-op）
...
部分 WPT 测试用 amoib 未实现的 API（如 `fetch` 需 mock HTTP 响应、某些依赖 `MessageChannel` 精确语义）。Runner 按结果分类：
- PASS — 通过
- FAIL — 失败（可能 API 差异，需 triage）
- ERROR/exception — harness 加载失败或 API 缺失（归类为 skip）

不追求 100% pass——WPT 是上游规范，amoib 的 WinterCG 子集会有已知差异。目标是**发现回归**（之前 pass 的变 fail）和**暴露未覆盖路径**。
...
## 验证
- spike 已验证 testharness.js 在 amoib 跑通
