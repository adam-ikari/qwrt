# Implementation Plan: amoib HTTPServer Extension (uvhttp, all features)

**Spec:** docs/superpowers/specs/2026-08-15-amoib-httpserver-design.md
**Status:** For review
...
- **P0.2** Check amoib's mbedtls config (`deps/mbedtls/include/mbedtls/mbedtls_config.h`)
  exposes what uvhttp's TLS/WS need (SHA1, base64, x509, SSLv3/TLS server) —
  note any missing defines.
- **P0.3** Confirm uvhttp compiles cleanly against amoib's mbedtls 3.6.6 headers
  (trial compile of uvhttp_tls.c / uvhttp_websocket.c with -I pointing at amoib's
  mbedtls include).
- **Verification:** `gcc -fsyntax-only` on uvhttp TLS/WS sources with amoib's
  mbedtls/miniz includes; list of sources per feature documented.

## Phase 1 — CMake integration

- **T1.1** Add `option(AM_WITH_HTTPSERVER ... ON)` + `set(UVHTTP_DIR ...)`
  next to existing options in root `CMakeLists.txt`; force
  `AM_WITH_TLS=ON` when HTTPSERVER=ON (mirror the existing TLS→CRYPTO force).
- **T1.2** When ON: append ext + uvhttp sources + deps (llhttp.c, xxhash.c,
  cJSON.c) to `amoib` library sources; add include dirs; define
  `AM_WITH_HTTPSERVER=1` + `UVHTTP_FEATURE_*`=1; link `uv_a`, mbedtls trio,
  `miniz`.
...
- **T2.1** `include/amoib/ext_http_server.h` — `extern const am_ext_t
  am_http_server_ext;`.
- **T2.2** `src/ext_http_server.c` — `#if AM_WITH_HTTPSERVER` wrapper; init
  callback registers `serve` global (via `JS_NewCFunction`); destroy callback
  stops/closes the uvhttp server; per-extension state (`am_ext_userdata`):
...
- **Verification:** CLI `amoib -e 'console.log(typeof serve)'` prints `function`
...
**Acceptance:** all integration tests pass; `AM_WITH_HTTPSERVER=OFF` build
