# amoib libuv-native 实现计划（PAL 删除 + Worker 支持）

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 amoib 从「宿主注入 loop + `am_tick` 轮询 + PAL 函数指针契约」迁移到「amoib 自建线程、自驱动 libuv loop、宿主仅通过 postMessage 消息通讯」，删除整个 PAL 抽象，并新增真线程 Web Worker 支持。

**Architecture:** 每个 `am_t` 由 `uv_thread_create` 创建独立线程，线程内自建并驱动 libuv loop（`uv_run(UV_RUN_ONCE)` 阻塞等事件 + 事件后 flush 微任务）。宿主 ↔ amoib 边界 = postMessage 语义：入站 `am_post_message(rt, json)` 线程安全入队，amoib 线程解析后派发成主 context 的 `onmessage` 事件；出站 amoib 线程同步调 `config->message_cb`。deferred 回调队列删除（libuv 回调与 JS 同线程）。多上下文机制保留但移出公共 API（由 polyfill 的 context.js 驱动）；Worker = 独立线程上的独立 `am_t`（独立 JSRuntime + loop），跨线程通讯用结构化克隆字节序列化。
...
以下约束来自 `docs/superpowers/specs/2026-08-12-amoib-libuv-native-design.md`，所有任务隐式包含本节。

- **平台**：仅 Linux + libuv（`uv_a`）是唯一后端；esp32/freertos/wasm/mock-PAL 全部删除。`platform/` 树删除。
- **执行模型 A**：amoib 自建线程（`uv_thread_create`）完全自驱动 loop；宿主**不碰 loop、不调 `am_tick`**。宿主回调跑在 amoib 线程上，**宿主回调必须线程安全**（W3C worker 语义）。
...
- **deferred 回调队列删除**：`am_defer_callback`、`deferred_cb_head/tail`、`pal_cb_data_t` 的 PAL 关联字段全部删除；libuv 回调直接 `JS_Call`。
- **公共 API 只保留消息交互**：删除 13 个函数（`am_tick`/`am_eval`/`am_eval_bytecode`/`am_call`/`am_reset`/`am_spawn`/`am_suspend`/`am_resume`/`am_destroy_ctx`/`am_get_active_ctx_id`/`am_get_jsctx`/`am_compile`/`am_compile_module`），宿主预编译用 `qjsc` CLI。新增 `am_post_message`。`am_create` 阻塞到 ready（mutex+condvar）；失败（init 异常 / initial_script eval 异常）→ 线程记录错误 → ready 握手带 fail code → `am_create` 返回 NULL。
- **多上下文**：机制保留（一个 am_t 持一个 JSRuntime + 多个 JSContext + 一个共享 loop），但移出公共 API，由 polyfill 的 `amContext` JS API 驱动；**宿主只见主 context**。
- **Worker**：`new Worker(url)` = 独立 OS 线程上的独立 `am_t`（独立 JSRuntime + 独立 loop），真实并行；`close()`/`terminate()`；worker 出站回到父 runtime 的入站队列（无宿主 message_cb）。多上下文 ≠ Worker（两套机制）。
...
- **代码规范**：严格 C99（`-std=c99`，`-Wall -Wextra -Werror`），4 空格无 tab，`snake_case`/`SHOUTING_CASE`，`/* */` 注释，无可变文件级全局（所有 per-runtime 状态在 `am_t` 上）。
- **测试**：离线确定性用 mock libuv（测试链接 `amoib` + `mock_libuv` + gtest + pthread）；gtest + `add_test`（NOT POST_BUILD）+ ctest labels `offline`/`network`/`benchmark`/`dap` 保留。测试命名 `TEST(am_create_*)`/`TEST(am_post_message_*)`/`TEST(host_*)`/`TEST(worker_*)`/`TEST(am_suspend_*)`/`TEST(am_resume_*)`。
...
现有 amoib API 未动，全树仍绿。等待用户指示（不 commit）。
```
...
**Files:**
- Rewrite: `include/amoib/amoib.h`（新公共 API，删 13 函数 + pal）
- Rewrite: `src/am_internal.h`（删 PAL/deferred 字段，加线程/队列/loop 字段）
- Create: `src/msgq.c`、`src/thread.c`
- Rewrite: `src/amoib.c`（生命周期 + post_message + runtime data）
- Modify: `src/context.c`（ctx 注册仍先于 polyfill 注入；`am_ctx_create` 不再带 pal）
- Modify: `src/bridge.c`（删 deferred 机制；`am_create_pal_object_ctx` 中异步 js_pal_* 存根；新增 `am_dispatch_message` + 出站 host codec）
- Create: `polyfill/src/host-messaging.js`（`postMessage`/`onmessage`/`__am_dispatch__`）
...
- Rewrite: `test/test_am_gtest.cpp`（迁到宿主契约）
- Modify: `test/CMakeLists.txt`（`add_am_gtest` 改链 `amoib mock_libuv`；临时禁用 http/stream/context/compress 套件并注明恢复任务）
...
```c
typedef struct am_config_s {
    const char *initial_script;      /* 主 context 启动时 eval；异常→am_create 返回 NULL */
    void (*message_cb)(am_t *rt, const char *json, size_t len, void *data);
    int  debug;                      /* 沿用 DAP bit 语义 */
    void *host_data;
} am_config_t;

am_t *am_create(const am_config_t *config);          /* 阻塞到 ready；失败 NULL */
void    am_destroy(am_t *rt);                          /* 宿主线程独占；优雅关停+join+free；NULL-safe */
int     am_post_message(am_t *rt, const char *json, size_t len);  /* 线程安全，任意线程 */
void   *am_get_runtime_data(am_t *rt);
void    am_set_runtime_data(am_t *rt, void *data);
void    am_free(void *ptr);                              /* 兼容语义：free 由 eval 等返回的 malloc 块；NULL-safe */
```

- Produces（内部）: `am_t` 新字段 `uv_loop_t loop; uv_async_t wake; uv_thread_t thread; uv_mutex_t msg_mutex; uv_cond_t ready_cond; uv_mutex_t ready_mutex; am_msg_t *msg_head,*msg_tail; int shutting_down; int thread_ready; int ready_err; am_config_t config;`；`am_msg_t { char *data; size_t len; int source; am_msg_t *next; }`（source: 0=host, >0=worker id）。`am_dispatch_message(rt, msg)`：host → `JS_JSONParse` → 调 polyfill 的 `__am_dispatch__(data, source)`；worker → `JS_NewArrayBufferCopy(bytes)` → 同上。
- Produces: polyfill 宿主模块 `polyfill/src/host-messaging.js`：定义 `globalThis.postMessage(data)`（→ `pal.postMessage(data)`）、`onmessage` setter（EventTarget 语义）、`__am_dispatch__(data, source)`（构造 `MessageEvent('message',{data})` 派发到 self）。`pal` 对象新增 `postMessage(value)`（bridge `js_pal_post_message`：`JS_JSONStringify` → `message_cb`）。

- [ ] **Step 1: 重写 amoib.h 公共 API**

删除：`am_pal_t`/`am_pal_cb_t`/`am_pal_stream_ops_t`/`am_pal_err_t`、`am_config_t.pal`、13 个函数声明、PAL 相关注释。保留 `am_t`/`am_ext_t`/`am_config_t`（改字段）/`am_create`/`am_destroy`/`am_get_runtime_data`/`am_set_runtime_data`/`am_free`。新增 `am_post_message`。`amoib.h` 顶部改为：
...
/* 创建 amoib：阻塞到内部线程 ready。initial_script 在 amoib 线程上 eval，
 * 抛异常则返回 NULL。宿主回调 message_cb 跑在 amoib 线程，必须线程安全。
 * 返回的 rt 由宿主线程调用 am_destroy 销毁。 */
am_t *am_create(const am_config_t *config);

/* 优雅关停：请求内部线程退出 → join → 释放 runtime → free。NULL-safe。
 * 只允许宿主线程调用（与 am_create 同一线程）。 */
...
/* 释放 amoib 分配的 malloc 块（历史兼容）。NULL-safe。 */
void am_free(void *ptr);

/* 扩展钩子（不变） */
typedef struct am_ext_s am_ext_t;
/* ...保留 am_ext_t / AM_EXTENSIONS 相关声明与注释... */
...
> 注：`am_ext_t` 的完整结构、`am_ext_registry.h` 宏、`struct JSContext` 前向声明照旧保留。`am_config_t` 旧字段 `pal` 删除后，`am_create` 内部不再校验 pal。

- [ ] **Step 2: 重写 am_internal.h**

删除：`pal_cb_data_t` 中的 `pal` 相关（保留 resolve/reject/ctx/rt/timer 字段）、`am_ctx_t.pal`、`am_t.deferred_cb_head/tail`、`has_pending_jobs`。新增：

```c
typedef enum { AM_MSG_SRC_HOST = 0 } am_msg_src_t;   /* >0 = worker id */

typedef struct am_msg_s {
    char *data;
    size_t len;
    int source;
    struct am_msg_s *next;
} am_msg_t;
...
struct am_t {
    ...
    void *host_data;
    int debug;
    /* 已有字段保留：contexts[AM_MAX_CONTEXTS], context_count, active_ctx_id,
     * WASM class IDs, dbg_session */
};
```

`am_ctx_t` 删除 `const am_pal_t *pal` 字段；其余（jsctx/context_id/active/suspended/handles/timer_resolves/timer_cbds/handle_count/extensions/polyfill）保留。`pal_cb_data_t` 改为 `am_cb_data_t`：
...
int am_msg_push(am_t *rt, const char *data, size_t len, int source) {
    am_msg_t *m = malloc(sizeof *m + len + 1);
    if (!m) return -1;
    m->data = (char *)(m + 1);
...
am_msg_t *am_msg_pop(am_t *rt) {
    uv_mutex_lock(&rt->msg_mutex);
    am_msg_t *m = rt->msg_head;
    if (m) { rt->msg_head = m->next; if (!rt->msg_head) rt->msg_tail = NULL; }
    uv_mutex_unlock(&rt->msg_mutex);
    return m;
}

void am_msg_free(am_msg_t *m) { free(m); }
```
...
static int am_flush_microtasks(am_t *rt) {
    int total = 0, n;
    while ((n = JS_ExecutePendingJob(rt->jsrt)) > 0) total += n;
    return total;
}

/* uv_async 回调：跑在 amoib 线程，排空入站队列并派发 onmessage */
static void am_wake_cb(uv_async_t *a) {
    am_t *rt = (am_t *)a->data;
    if (rt->shutting_down) return;
    am_msg_t *m;
    while ((m = am_msg_pop(rt)) != NULL) {
        am_dispatch_message(rt, m);   /* bridge.c 实现；JSRuntime 已就绪 */
        am_msg_free(m);
    }
}

static void am_thread_main(void *arg) {
    am_t *rt = arg;
    ...
    rt->wake.data = rt;
    if (uv_async_init(&rt->loop, &rt->wake, am_wake_cb) != 0) goto fail_loop;

    /* 完整初始化（沿用旧 am_create 主体，去掉 pal）：JS_NewRuntime、
     * JS_SetRuntimeOpaque、扩展 init、am_ctx_create、polyfill 注入、DAP attach。
     * initial_script 此时在活动 context 上 eval。 */
    rt->jsrt = JS_NewRuntime();
    if (!rt->jsrt) goto fail_loop;
    JS_SetRuntimeOpaque(rt->jsrt, rt);
    /* ...am_runtime_init(rt)（见 amoib.c 重构后的内部函数）... */
    if (rt->config.initial_script) {
        char *err = NULL;
        if (am_eval_internal(rt, rt->config.initial_script, &err) != 0) {
...
handshake_and_exit:
    ...
    if (rt->ready_err) {
        /* init 失败：清理后线程自己退出；am_create 返回 NULL */
...
> 注：`am_dispatch_message` 实现放 bridge.c（Step 6）；`am_runtime_init`/`am_eval_internal`/`am_thread_teardown` 放 amoib.c 重构（Step 5），签名在下方给出。`rt->wake.data` 需要 `uv_async_t` 有 `data` 字段——mock 与真实 libuv 的 `uv_async_t` 都有 `void *data`。

- [ ] **Step 5: 重写 src/amoib.c 生命周期**

```c
#include "am_internal.h"

static int am_runtime_init(am_t *rt);
static void am_thread_teardown(am_t *rt);
static void am_thread_main(void *arg);   /* thread.c 定义 */

am_t *am_create(const am_config_t *config) {
    if (!config) return NULL;
    am_t *rt = calloc(1, sizeof *rt);
    if (!rt) return NULL;
    rt->magic = AM_MAGIC;
    rt->config = *config;
    ...
    uv_mutex_unlock(&rt->ready_mutex);
    if (rt->ready_err) { am_destroy(rt); return NULL; }
    return rt;
}

int am_post_message(am_t *rt, const char *json, size_t len) {
    if (!rt || rt->magic != AM_MAGIC || !json) return -1;
    return am_msg_push(rt, json, len, AM_MSG_SRC_HOST);
}
...
void am_dispatch_message(am_t *rt, am_msg_t *m) {
    ...
    JSValue fn = JS_GetPropertyStr(ctx, JS_GetGlobalObject(ctx), "__am_dispatch__");
...
```js
// host-messaging.js — 宿主↔amoib 消息的 JS 面（W3C worker 语义）
...
// 宿主→JS：bridge 调 __am_dispatch__(data, source)；source 0=host
...
- [ ] **Step 9: 重写 test/test_am_gtest.cpp（宿主契约核心套件）**
...
TEST(host_, message_thread_safety) {
    ...
    for (int i = 0; i < N; i++) ts.emplace_back([h, i]{
        for (int k = 0; k < PER; k++) {
            ...
            am_post_message(h->rt, s.data(), s.size());
        }
    });
    int seen = 0;
    std::string out;
...
`add_am_gtest` 内 `target_link_libraries(${name} amoib am_mock GTest::gtest_main pthread)` 改为 `amoib mock_libuv GTest::gtest_main pthread`，并把 `test/CMakeLists.txt` 里的 `am_mock` 引用全部换成 `mock_libuv`。**临时注释**以下套件的 `add_am_gtest(...)` 行并注明恢复任务（Task 3 恢复 bridge/fetch/polyfill，Task 5 恢复 context/compress）：
...
> 注：这些套件引用已删除的 `am_eval`/`am_spawn` 等符号，Task 2 必须先从构建中摘除才能保持全树绿。Task 3/5 用 host_eval 通道重写后放回。
...
```text
执行模型 A 落地：amoib.h 新公共 API（am_create 阻塞式 + am_post_message + 13 函数删除）、
src/thread.c 线程主循环（uv_run ONCE + wake_cb 派发 + 微任务 flush）、src/msgq.c 入站 FIFO、
src/amoib.c 生命周期重写（ready 握手 + 优雅关停 join + NULL-safe）、bridge.c 删 deferred、
polyfill host-messaging.js（postMessage/onmessage/__am_dispatch__）、test_host.h 宿主测试桩、
test_am_gtest 迁到宿主契约（message roundtrip / eval 通道 / 线程安全 / destroy join）。
...
- Modify: `test/CMakeLists.txt`（恢复 bridge/fetch/polyfill 套件；加 `src/uv_io.c` 进 amoib 库）
...
**Interfaces:**
- Consumes: Task 2 的 `am_cb_data_t`（bridge 持有 resolve/reject + ctx + rt + handle_idx）、mock 扩展的 uv_*。
- Produces: `uv_io_*` 后端函数（内部，bridge.c 调用）。统一完成回调签名 `typedef void (*am_io_done_t)(void *opaque, int status, const char *result, size_t len);`。`opaque` = bridge 的 `am_cb_data_t*`（含 resolve/reject）；回调在 amoib 线程（libuv 回调上下文）直接 `JS_Call` resolve/reject——**不再经 deferred**。
- 关键签名（从 pal_uv.c 同名函数平移，`pal` 参数替换为 `rt`）：

```c
void uv_io_timer_start(am_t *rt, uint64_t timeout_ms, uint64_t repeat_ms, am_io_done_t done, void *opaque); /* 一次性：到期调 done(status=0) */
void uv_io_timer_stop(am_t *rt, int handle_idx);
void uv_io_http_request(am_t *rt, const char *method, const char *url, const char *headers_json, const uint8_t *body, size_t body_len, am_io_done_t done, void *opaque);
void uv_io_http_request_stream(am_t *rt, const char *method, const char *url, const char *headers_json, const uint8_t *body, size_t body_len, am_pal_stream_ops_t *ops);
void uv_io_fs_read(am_t *rt, const char *path, am_io_done_t done, void *opaque);
void uv_io_fs_write(am_t *rt, const char *path, const uint8_t *data, size_t len, am_io_done_t done, void *opaque);
void uv_io_fs_exists(am_t *rt, const char *path, am_io_done_t done, void *opaque);
void uv_io_fs_remove(am_t *rt, const char *path, am_io_done_t done, void *opaque);
void uv_io_fs_list(am_t *rt, const char *path, am_io_done_t done, void *opaque);
void uv_io_storage_get(am_t *rt, const char *key, am_io_done_t done, void *opaque);
void uv_io_storage_set(am_t *rt, const char *key, const uint8_t *data, size_t len, am_io_done_t done, void *opaque);
void uv_io_storage_del(am_t *rt, const char *key, am_io_done_t done, void *opaque);
void uv_io_random_bytes(am_t *rt, uint8_t *buf, size_t len);
uint64_t uv_io_time_now(am_t *rt);
uint64_t uv_io_hrtime(am_t *rt);
void uv_io_log(am_t *rt, int level, const char *msg);
```

- [ ] **Step 1: 写 src/uv_io.c 骨架 + 同步函数**

```c
/* uv_io.c — amoib 直连 libuv 的 I/O 后端（PAL 删除后的替代层）。
 * 所有回调跑在 amoib 线程（libuv 事件循环线程），可直接 JS_Call，无 deferred。
 * 从 platform/uv/pal_uv.c 吸收并去 pal 化：pal 参数→rt，存储用 am_t 内的表。 */
#include "am_internal.h"
#include <stdio.h>
#include <string.h>

uint64_t uv_io_time_now(am_t *rt) { (void)rt; return uv_now(&rt->loop); }
uint64_t uv_io_hrtime(am_t *rt) { (void)rt; return uv_hrtime(); }
void uv_io_log(am_t *rt, int level, const char *msg) {
    (void)rt; fprintf(stderr, "[amoib:%d] %s\n", level, msg ? msg : "");
}
void uv_io_random_bytes(am_t *rt, uint8_t *buf, size_t len) {
...
- [ ] **Step 2: timer 后端（uv_timer_t 注册进 am_t 的 handle 表）**

```c
/* 沿用旧 handle 表模型：am_ctx_t->handles[] 存 uv_timer_t*，timer_resolves[] 存 JSValue。 */
void uv_io_timer_start(am_t *rt, uint64_t timeout_ms, uint64_t repeat_ms, am_io_done_t done, void *opaque) {
...
> 注：`pal_uv.c` 的 `pal_uv_fs_read/write/exists/remove/list` 已有完整两段式实现（open→read 等）。**执行者打开 `platform/uv/pal_uv.c`（3342 行）逐函数平移**：去 `pal` 参数、去 `pal_uv_t` 存储（用 `am_t`/静态表）、回调改 `am_io_done_t`。storage 用旧 storage 表逻辑（`pal_uv_store_entry_t` 平移到 am_t 或 uv_io 内部静态表——**注意无可变文件级全局约束**，storage 表放 `am_t` 新增字段）。TLS 分支由 `AM_WITH_TLS` 控制（mbedTLS，沿用 pal_uv_http_request 内嵌 TLS 栈）。

- [ ] **Step 4: http 后端（uv_tcp + TLS，从 pal_uv.c 平移）**

平移 `pal_uv_http_request`/`pal_uv_http_request_stream`/`getaddrinfo`/`connect`/`read`/`parse`（chunked 解码等 bug 已修，见记忆 [[amoib-pal-uv-pending-bugs]]，平移时保留修复）。完成/流式回调改 `am_io_done_t`/`am_pal_stream_ops_t`（后者结构保留在 am_internal.h，不再从 amoib.h 导出）。
...
static JSValue js_pal_fs_read(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    am_cb_data_t *cd = alloc_cb_data(cctx, promise, 1 /*resolve idx*/, cctx->rt);
...
> 桥仍只做三件事：`JS_ToCString` 转换 → `uv_io_*` 调用 → `JS_NewPromiseCapability` + 完成回调里 resolve/reject。路径校验 `bridge_validate_path` 保留。`alloc_cb_data`/`am_free_cb_data` 保留（去 pal 字段后即为 `am_cb_data_t`）。http/stream/storage 同模式平移。
...
`test/CMakeLists.txt` 取消 Task 2 的注释；把四个套件（bridge_http/bridge_stream/fetch_stream/polyfill）的 `am_create(pal)`+`am_eval` 用法迁到 `host_eval`/`host_create` 契约（机械替换：`am_create` 带 mock pal → `host_create(bootstrap)`；每个 `am_eval(rt, code)` → `host_eval(h, code, &out)` 再解析 `out`）。polyfill 套件中直接测底层 `pal.*` 的用例改为经命令通道 `eval("pal.fs_read(...)")`。
...
**Files:**
- Create: `src/worker.c`（`am_worker_t` + worker 线程主循环 + 父↔子队列桥）
- Modify: `src/am_internal.h`（worker 相关内部类型、`am_msg_src_t` 扩展）
- Modify: `src/thread.c`（`am_wake_cb` 派发支持 worker source；worker 消息路由到对应 Worker 实例）
- Modify: `src/bridge.c`（`am_dispatch_message` worker 分支已有；补 worker 相关的 pal JS 函数：`pal.spawn_worker`/`pal.worker_post`/`pal.worker_terminate`）
...
**Interfaces:**
- Consumes: Task 2 的入站 FIFO + `am_dispatch_message` 的 worker 分支、Task 3 的 mock uv_async/thread/cond。
- Produces（C 内部）:

```c
typedef struct am_worker_s am_worker_t;
am_worker_t *am_worker_create(am_t *parent, const char *script, int *out_err); /* script=源码文本 */
void am_worker_post(am_t *parent, am_worker_t *w, const uint8_t *bytes, size_t len); /* 父→子 */
void am_worker_terminate(am_t *parent, am_worker_t *w);   /* 异步：发关停信号，不阻塞父 JS */
...
> 注：格式自定但必须支持循环引用与 TypedArray/ArrayBuffer/Blob 等现有 `structuredClone` 已支持的类型；不支持函数/符号（抛 `DataCloneError`）。**执行者参考现有 `polyfill/src/structured-clone.js` 的 `cloneValue` 分支表，一一对应到编码/解码两遍**。现有 `globalThis.structuredClone` 保留（任务内进程克隆），新函数挂 `globalThis.__am_serialize__`/`__am_deserialize__`（或同模块导出供 worker.js 用）。
...
struct am_worker_s {
    ...
    /* worker 自己的线程/loop/runtime（复用 am_thread_main 模式，但宿主=父 runtime） */
    uv_thread_t thread;
    uv_loop_t loop;
    uv_async_t wake;
    uv_mutex_t msg_mutex;
    am_msg_t *msg_head, *msg_tail;   /* 父→子 入站 */
    int shutting_down;
    am_t *self;                 /* worker 的 am_t（独立 JSRuntime） */
    char *script;
};
...
static void am_worker_thread_main(void *arg) {
    ...
    /* 主循环（同 am_thread_main）：wake_cb 排空父→子入站并派发 onmessage；
...
`js_pal_worker_post`：`argv[0]`=worker id，`argv[1]`=ArrayBuffer（字节）→ `am_worker_post(rt, w, bytes, len)`。worker 侧 JS 的 `postMessage(value)` 需先 `serializeToBytes(value)` 再经 worker 自己的 pal 通道；C 层区分：worker 的 `pal.worker_post` 目标是父 runtime（bridge 在 worker 的 am_t 里把 `postMessage` 绑到 `am_worker_post` 的父入队路径）。**子→父 不设 message_cb**（spec 硬性）。
...
Worker.prototype.postMessage = function (value) {
  var bytes = __am_serialize__(value);
...
// bridge 派发 worker 消息（source>0）时调 __am_dispatch__，这里路由到实例：
globalThis.__am_dispatch__ = function (data, source) {
  if (source === 0) {
    self.dispatchEvent(new MessageEvent('message', { data: data }));
  } else {
    var w = workers.get(source);
    if (w && w._onmsg) w._onmsg(new MessageEvent('message', { data: __am_deserialize__(data) }));
  }
};
```

> 注：`__am_dispatch__` 在 host-messaging.js（Task 2）里定义，这里**覆盖为带 worker 路由的版本**（或 host-messaging 直接按 `source` 判断）。`close()`：worker 内调 `postMessage` 后退出自身——`pal.worker_close(wid)`（C：发关停信号）。
...
**Files:**
- Modify: `src/context.c`（内部多上下文机制保留；新增挂起/恢复辅助 `am_ctx_serialize`/`am_ctx_rebuild`；删 `am_reset`/`am_suspend`/`am_resume`/`am_destroy_ctx`/`am_get_active_ctx_id`/`am_get_jsctx` 公共实现）
- Modify: `src/bridge.c`（pal JS 函数：`pal.context_spawn`/`pal.context_suspend`/`pal.context_resume`/`pal.context_destroy`）
- Create: `polyfill/src/context.js`（`amContext` JS API：spawn/suspend/resume/destroy，宿主无感知）
...
- Create: `test/test_suspend_gtest.cpp`（`TEST(am_suspend_*)`/`TEST(am_resume_*)`）
- Modify: `src/am_internal.h`（`am_ctx_t` 加 `polyfill` 重注入 + 序列化状态字段）

**Interfaces:**
- Consumes: Task 2/3 的 fs（`uv_io_fs_write`/`uv_io_fs_read` 落盘 manifest）、structured-clone 字节（Task 4）。
- Produces（polyfill JS）: `amContext`（`globalThis.amContext = { spawn(initScript), suspend(ctxId), resume(ctxId, scriptRef), destroy(ctxId) }`）；挂起 = 序列化可克隆全局属性 → 结构化克隆字节 → 写 fs manifest `{context_id, script_ref, state_file, timestamp}`；恢复 = 重建 JSContext + 重注入 polyfill + 重 eval 脚本 + 反序列化回 globals。在途异步不保留（spec）。
- Produces（C 内部）: `int am_ctx_serialize(am_t *rt, int ctx_id, const char *state_path)`（收集 `globalThis` 可克隆属性 → 字节 → `uv_io_fs_write`）、`int am_ctx_rebuild(am_t *rt, int ctx_id, const char *script_ref, const char *state_path)`（重建 + 反序列化）。
...
int am_ctx_serialize(am_t *rt, int ctx_id, const char *state_path) {
    am_ctx_t *cctx = am_find_ctx(rt, ctx_id);
    if (!cctx) return -1;
    JSContext *ctx = cctx->jsctx;
...
int am_ctx_rebuild(am_t *rt, int ctx_id, const char *script_ref, const char *state_path) {
    /* 1) 重建 JSContext（am_ctx_create 无 pal 版）+ 重注入 polyfill
...
> 注：`am_find_ctx` 从现有 `get_ctx_from_jsctx` 同族逻辑抽一个按 id 查找。全局属性过滤规则：排除 `__pal__`/`__pal_inject__`/`__am_dispatch__` 等内建、函数、WASM 类；可克隆判定用 Task 4 序列化内核的容错（不可克隆属性跳过并在 manifest 记 `skipped` 清单）。

- [ ] **Step 2: bridge pal context 函数 + context.js**

```js
// context.js — amContext：多上下文 + 软挂起/恢复（宿主只见主 context）
var pal = globalThis.__pal_inject__;
var ctxSeq = 1;
globalThis.amContext = {
  spawn: function (initScript) {
    // pal.context_spawn(initScript) → C 里 am_ctx_create（内部 context 表）
...
`test/CMakeLists.txt` 取消 Task 2 注释；`test_context_gtest.cpp` 改用 `host_eval` 经 `amContext.spawn/suspend/resume` 驱动多上下文（如：spawn 一个子 context 挂载脚本，宿主在主 context 发消息确认两边隔离）。`test_compress_gtest`/`test_compress_consistency_gtest` 仅需机械迁到 `host_eval`（压缩本身同步，无 I/O）。

```cpp
TEST(am_suspend_, serialize_global_state) {
    HostCtx *h = host_create();
    ASSERT_NE(nullptr, h);
    std::string out;
    // 主 context 里定义全局状态 → amContext.suspend 到测试目录
    ASSERT_TRUE(host_eval(h,
        "globalThis.foo = {n: 42, s: 'x'};"
        "amContext.suspend(0, '" TEST_DIR "/state.bin'); 'ok'", &out));
    host_destroy(h);
}
TEST(am_resume_, restore_global_state) {
    HostCtx *h = host_create();
    ASSERT_NE(nullptr, h);
    std::string out;
    // 先 suspend 再 resume 到新 context，确认 foo 回来了
    ASSERT_TRUE(host_eval(h,
        "amContext.resume(0, '" TEST_DIR "/script.js', '" TEST_DIR "/state.bin');"
...
```text
多上下文移入内部 + polyfill amContext JS API（spawn/suspend/resume/destroy）。
...
**Files:**
- Delete: `include/amoib/am_pal.h`、`platform/`（uv/mock/freertos/wasm 全树）、`docs/pal-design.md`、`docs/esp32s3-design.md`、`docs/pal/`（若存在）
- Modify: `CMakeLists.txt`（删 `AM_PAL_*` 选项、`am_uv`/`am_mock`/`am_freertos`/`am_pal_common`/`am_wasm` 目标、`libam_full` 改链 `amoib + real libuv + mbedtls`；`uv_a` 为硬依赖；`amoib` 静态库只编译 src/*.c 不链 uv——最终目标再解析符号）
- Modify: `test/CMakeLists.txt`（删所有 `am_mock`/`am_pal_common` 引用与 `add_am_pal_common_gtest`、`test_pal_common_gtest`；删 orphaned 纯 C 测试文件的构建条目——先核对哪些已无 add_executable 引用）
...
**Interfaces:**
- Consumes: 前五任务的产物；`CMakeLists.txt` 现有 `am_enable_warnings` 等辅助宏保留。

- [ ] **Step 1: 删 PAL 源树与头文件**

Run: `git rm -r platform include/amoib/am_pal.h`（如文件未追踪则普通 `rm`；删除前 `git status` 确认为受版本控制）。核对 `grep -rn "am_pal\|pal_uv\|pal_mock\|pal_freertos\|AM_PAL" src include test polyfill docs CMakeLists.txt` 不再有引用（bridge.c 的 `am_pal_stream_ops_t` 已迁到 am_internal.h，本任务从 am_internal.h 改名 `am_io_stream_ops_t` 或在内部保留原结构体名——**选择保留内部名，避免无谓改名**；`amoib.h` 已无任何 pal 符号）。

- [ ] **Step 2: 清 CMakeLists.txt 与 test/CMakeLists.txt**

删选项与目标；`libam_full` 链接 `amoib uv_a mbedtls mbedx509 mbedcrypto miniz vmlib rt dl pthread`（`AM_WITH_TLS` 等特性开关保留）。`add_am_gtest` 链接 `amoib mock_libuv GTest::gtest_main pthread`（Task 2 已改）。确认 `AM_WITH_WAMR`/`AM_WITH_WASM3`/`AM_WITH_COMPRESS`/`AM_WITH_CRYPTO_EXT`/`AM_WITH_TEXTCODEC` 与扩展目标不动。
...
CLAUDE.md：Build & Test 节——`AM_PAL_*` 移除、测试链接说明改 `amoib + mock_libuv`、新增「`amoib` 静态库不链 uv，最终目标解析符号」说明；Architecture 节——替换执行模型 A 描述（自建线程/自驱 loop/消息边界/deferred 删除）、文件清单加 `src/uv_io.c`/`src/thread.c`/`src/msgq.c`/`src/worker.c`、删 `platform/` 与 PAL 段落、`include/amoib/am_pal.h` 条目删除。README 同步（若含 PAL 段落）。

- [ ] **Step 5: 删除文档**

`git rm docs/pal-design.md docs/esp32s3-design.md`；`docs/pal/` 若存在 `git rm -r docs/pal`。`docs/amoib-architecture-design.md` 里 PAL 相关章节标记为已废弃或更新（按实际内容小改）。
...
```text
PAL 全删：platform/ 树、am_pal.h、AM_PAL_* 选项、am_uv/mock/freertos/pal_common 目标、
orphaned 纯 C 测试（test_qwrt.c 悬空 include 一并清除）、docs/pal-design.md + esp32s3-design.md。
CLAUDE.md/README 更新为新执行模型。libam_full 链 real libuv。全量离线测试 + valgrind 绿。
等待用户指示（不 commit）。
```
...
- §5 消息边界 → Task 2（host JSON 无信封 + bad-json 分支在 `am_dispatch_message`/JS 侧兜底：本计划 bad-json 由 `JS_JSONParse` 异常 → `JS_Call` 失败忽略——**补**：按 spec 应在 message_cb 发 `{"type":"error","error":"bad-json"}`，执行时在 `am_dispatch_message` 的 JSON parse 失败分支补上该回调）；worker 字节 → Task 4。
- §6.1 多上下文 → Task 5（移出公共 API + amContext JS API）。
...
- §7 删除清单 → Task 2（13 函数 + amoib.h pal）、Task 6（platform/am_pal.h/选项/文档/CLAUDE.md）；`am_defer_callback`/deferred 字段 → Task 2 Step 6。
- §8 修改与新增清单 → Task 1–6（mock_libuv、uv_io、thread、msgq、worker、context、structured-clone、host-messaging、worker.js、context.js）。
- §9 测试策略 → 各任务（命名 `TEST(am_create_*)`/`host_*`/`worker_*`/`am_suspend_*`/`am_resume_*`；labels 保留）。
...
**Type consistency：** `am_post_message`/`am_config_t`/`am_cb_data_t`/`am_msg_t`/`am_worker_t`/`uv_io_*`/`__am_dispatch__(data, source)`/`serializeToBytes`/`deserializeFromBytes` 在定义处与后续使用处签名一致。`am_free_cb_data`/`alloc_cb_data` 沿用旧名（Task 2 改名 `am_cb_data_t` 但函数名不变，Task 3 用到，已一致）。
...
4. `libam_full` 从「core+uv+mock+extensions」改为「amoib（不含 uv）+ real libuv + 扩展」，符号在最终链接解析。
