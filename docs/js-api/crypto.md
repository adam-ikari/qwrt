...
All SubtleCrypto methods return `Promise`s. Available algorithms depend on whether `QZ_WITH_CRYPTO_EXT` is enabled at build time.
...
AES encryption/decryption. Available when `QZ_WITH_CRYPTO_EXT=ON`.
...
Available when `QZ_WITH_CRYPTO_EXT` provides PBKDF2 support.
...
`exportKey` / `wrapKey` / `unwrapKey` / `deriveKey` are available when `QZ_WITH_CRYPTO_EXT=ON`.
...
When `QZ_WITH_CRYPTO_EXT=OFF`, only `crypto.getRandomValues()` is available. `crypto.subtle` is `undefined` — there is no JS fallback.
