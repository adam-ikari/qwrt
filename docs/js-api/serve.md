...
The WebSocket handshake requires `QZ_WITH_TEXTCODEC=ON` and
`QZ_WITH_CRYPTO_EXT=ON` at build time (SHA-1 accept key via `crypto.subtle`);
...
Passing `tls: { cert, key }` enables HTTPS on the listener. Requires `QZ_WITH_TLS=ON` at build time (see [Build Options](/guide/build-options)).
...
Requires `QZ_WITH_GRPC=ON` at build time (the gRPC bundle is opt-in because
...
[grpc-hello example](https://github.com/adam-ikari/qzjs/tree/master/examples/grpc-hello)
