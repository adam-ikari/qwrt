...
description: The Service Worker subset in Qwrt.js — registration, fetch interception with Cache API, and update mechanism. Exposed as navigator.serviceWorker.
---
...
the SW lifecycle, and installs `__qwrt_sw_intercept__` as the fetch hook.
...
- **Activate**: after install, the SW becomes the controller; `skipWaiting`
  shortens the wait (qwrt installs and activates immediately — there is no
  old-controller handover needed in a single-runtime embed).
- **Fetch interception**: once activated, every `fetch()` in the runtime
  goes through `__qwrt_sw_intercept__`; the SW's `fetch` handler decides
...
- Global registration: there is exactly one SW per runtime; `scope` is
  accepted but ignored (qwrt treats the runtime as one origin).
