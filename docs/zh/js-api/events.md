...
description: Qzjs.js 中的 EventTarget 和 Event API —— addEventListener、dispatchEvent、CustomEvent 以及事件传播。
---
...
let cancelable = new Event('submit', {
    ...
    composed: true        // 跨越 shadow DOM 边界（在 qzjs 中不相关）
});
```
...
由于 qzjs 没有 DOM 树，冒泡/捕获只有在你构建自己的事件层次结构时才有意义。
