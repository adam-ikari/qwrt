...
qzjs 运行多个独立的 JS 执行上下文。**面向宿主**的并行方式是标准的
...
`-DQZ_PROCESS_MODEL` 构建选项控制 worker 如何运行：
...
在 C 层，qzjs 维护一组隔离上下文（`qz_ctx_t`），同一时刻只有一个活动
上下文。上下文拥有独立的全局、原生状态与扩展状态，可软挂起/恢复到磁盘。这套
机制是**内部的** — 没有公开的宿主 API 来派生/挂起/恢复上下文。它用于支撑
`new Worker` 与扩展生命周期，并通过内部上下文辅助
（`qz_get_active_ctx`、`qz_get_active_jsctx`、`qz_get_ctx_by_id`，
位于 `src/context.c`）暴露给编译进 qzjs 的 C 扩展。
