...
```c
qz_config_t config = {
    .initial_script = "postMessage('ready');",  // 在创建时于 qzjs 线程上求值
...
- **所有 JS 在 qzjs 的内部线程上运行** — 宿主线程从不调用 JS
- **`qz_post_message` 是线程安全的** — 可从任何线程调用；JSON 会被拷贝
- **`message_cb` 在 qzjs 线程上触发** — 你的回调必须线程安全
