...
description: Parallel execution in Amoib.js — new Worker(url), isolated contexts, and the ISOLATED vs THREAD process models.
---

# Multi-Context & Web Workers

amoib runs multiple independent JS execution contexts. The **host-facing** way
...
The `-DAM_PROCESS_MODEL` build option controls how a worker runs:

| Model | Worker execution |
|-------|------------------|
| `THREAD` | a parallel thread in the same process |
| `ISOLATED` (default) | a dedicated child process (`amoib-rt`, spawned via fork+exec) |
...
At the C layer amoib maintains a set of isolated contexts (`am_ctx_t`) with a
...
extensions (built into amoib) through the internal context helpers
(`am_get_active_ctx`, `am_get_active_jsctx`, `am_get_ctx_by_id` in
`src/context.c`).
