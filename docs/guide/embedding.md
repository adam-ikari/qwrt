...
description: Patterns for embedding Amoib.js in C applications — host data, custom extensions, message-based communication, and multi-instance setups.
---

# Embedding Patterns

Common patterns for embedding amoib in C applications.

## Basic Embedding

amoib owns its own internal thread and libuv event loop. All JS runs on that
...
`am_create` blocks until amoib's internal thread is ready and
`initial_script` has been eval'd. There is no `am_eval` and no `am_tick` —
the host sends messages via `am_post_message` (thread-safe) and receives
replies through `message_cb`, which fires on the amoib thread (so your callback
must be thread-safe). `am_destroy` performs a graceful shutdown.
...
The `init` hook runs on amoib's internal thread during `am_create`, before
the host receives the runtime — so registering globals here is safe.
`am_get_active_jsctx` is an internal helper (declared in
`src/am_internal.h`), for use from extension hooks.
...
am_post_message(rt, "{\"cmd\":\"process\",\"value\":21}", 28);
// on_message prints: JS returned: {"doubled":42,"ok":true}
```

The JSON is copied by `am_post_message` (thread-safe, callable from any
thread). There is no synchronous `am_call` — results always flow back as
messages.
...
communicates over JSON messages (`am_post_message` / `message_cb`). For
request-level isolation, either create a fresh `am_t` per request (each is
...
Gtest suites link `amoib + mock_libuv` (with `-DAM_USE_MOCK_LIBUV`) and drive
...
Since amoib has zero global state, you can run multiple `am_t` instances —
...
am_config_t cfg1 = { .initial_script = "postMessage('rt1');",
                       .message_cb = on_message, .host_data = "rt1" };
am_config_t cfg2 = { .initial_script = "postMessage('rt2');",
                       .message_cb = on_message, .host_data = "rt2" };

am_t *rt1 = am_create(&cfg1);
am_t *rt2 = am_create(&cfg2);

// Post to each independently; replies arrive on each runtime's message_cb
am_post_message(rt1, "{\"cmd\":\"echo\",\"data\":\"a\"}", 26);
am_post_message(rt2, "{\"cmd\":\"echo\",\"data\":\"b\"}", 26);
...
- If `initial_script` throws, `am_create` returns `NULL`.
...
- `am_free` still exists for malloc'd blocks returned by amoib (`am_free(NULL)` is safe) — there is no longer any `am_eval`/`am_call` result to free
- The runtime owns all its internal resources (thread, libuv loop, contexts) — `am_destroy` frees everything on graceful shutdown
- Per-runtime host data: set `config.host_data` before `am_create`; an
  extension's `init` hook reads it via `am_get_runtime_data(rt)` during
  create (the rt is valid inside init, before the host receives it). Note:
  `am_ext_t.user_data` is on the shared compile-time extension struct — use
  `am_get_runtime_data`/`am_set_runtime_data` for per-instance data, not
