...
description: How Amoib.js uses QuickJS bytecode internally (qjsc) to speed startup — and why there is no public host-side bytecode-loading API.
---

# Bytecode Compilation

amoib precompiles its own JavaScript (the WinterTC polyfill and the worker boot
...
```bash
# amoib's build does this for the WinterTC polyfill and worker boot script
qjsc -c polyfill.js -o polyfill_bytecode.c
```

At runtime the embedded bytecode is evaluated on the internal thread instead of
parsing source. This is a amoib-internal optimization — the bytecode is produced
from amoib's own sources and never exposed to hosts.

## No Public Bytecode API

There is **no public `am_compile` / `am_eval_bytecode`** in `amoib.h`, and
the `amoib` CLI has no bytecode option. A host cannot hand amoib a bytecode blob
...
(`am_eval_bytecode_internal` in `src/am_internal.h`), used by amoib's own
runtime and by C extensions built into amoib. If you are writing such an
...
in `initial_script` and let amoib's precompiled polyfill carry the cost. For
