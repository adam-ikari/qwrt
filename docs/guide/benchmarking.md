...
description: Amoib.js performance benchmark suite — six CI-driven benchmarks covering HTTP server, HTTP/2 client, worker runtime, cross-runtime comparison, JS API primitives, and TLS/WS/gRPC servers, with a committed baseline and per-metric Δ% drift reporting.
---

# Performance Benchmarks

amoib ships a benchmark suite that runs entirely in CI. Every push to `master`
...
| `cross-runtime` | `test/bench_cross_runtime.py` | amoib vs node vs bun: startup / eval / RSS, plus `--js-api` extension running `bench_js_api.mjs` on all three. Record-only. |
| `js-api-perf` | `test/bench_js_api.mjs` | Five JS-API modes (streams / crypto / compress / fs / wasm) × amoib/node/bun. Record-only. |
| `tls-ws-grpc-perf` | `test/bench_tls_server.py` + `bench_ws_server.mjs` + `bench_grpc_unary.py` | TLS server (wrk), WebSocket echo, gRPC unary. amoib-native. Record-only. |
...
Baselines live in [`test/perf_baselines/`](https://github.com/adam-ikari/amoib/tree/master/test/perf_baselines):

```
test/perf_baselines/
├── js-api.json          # amoib/node/bun × 5 modes, all metrics
...
./build/amoib test/bench_js_api.mjs --mode all --iters 0.25   # interpreter
...
# gRPC unary (needs AM_WITH_GRPC=ON build)
python3 test/bench_grpc_unary.py --amoib-bin ./build/amoib --calls 2000
```

The `--iters` flag scales the inner workload. amoib (interpreter, no JIT) uses
