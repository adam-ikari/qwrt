...
JS 层契约（Worker API、postMessage、structuredClone、MessagePort）在双模型下**完全一致**——差异只隔离在 C 层 qwrt_t 的消息后端。这是设计铁线：任何 JS 改动都视为越界，除非被 §8 MessagePort 跨进程语义明确豁免。
...
## M-P1：worker 进程 spawn + fd 通道 + 握手 + 消息往返
...
1. **默认 ISOLATED 的性能取舍**：接受「worker 消息 P99 比线程慢约 10~50×」去换崩溃隔离？（§1.2/§12.2——虽然方向已定 ISOLATED 默认，但这里量化了成本，若边缘场景追求极致吞吐可能想关）——**倾向：维持 ISOLATED 默认**，成本在可接受区间。
...
> 附件/引用：`docs/plans/2026-09-03-flatbuffers-runtime-builtin.md`（fb 定位 + 升格触发条件，本设计即该触发）；`docs/plans/2026-09-03-grpc-http2-design.md`（风格参照）；`src/qwrt.c` / `src/thread.c` / `src/worker.c` / `src/msgq.c` / `src/cli.c`（现状架构）。