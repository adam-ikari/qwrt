/* __qwrt_serialize__/__qwrt_deserialize__ 字节等价性语料测试：
 * 新旧 polyfill 对同一语料库应产出完全相同字节（hex），且往返一致。
 * 用法：./build/qwrt test/bench/serialize_corpus.js
 */
function hex(buf) {
...
for (var i = 0; i < corpus.length; i++) {
  try {
    var bytes = __qwrt_serialize__(corpus[i]);
    var back = __qwrt_deserialize__(bytes);
