#!/usr/bin/env python3
"""qzjs cross-runtime performance benchmark (cold start / eval / peak RSS).

Compares qzjs against other JS runtimes on the metrics that define its
...
(Node worker_threads, Bun Worker, qzjs dual-backend) — that axis stays
qzjs-internal (bench_runtime.py).

Usage:
  python3 test/bench_cross_runtime.py \
      --bins qzjs=./build/qzjs,node=$(which node),bun=$(which bun) \
      [--quick] [--json out.json]

--bins entries whose binary is missing on the host are skipped. Ratios in
the JSON summary are computed vs qzjs when qzjs is present. The JSON
...
# tjs/qjs share the QuickJS lineage with qzjs.
ORDER = ['qzjs', 'node', 'bun', 'tjs']
ALIAS = {'qzjs': 'qzjs', 'node': 'node', 'bun': 'bun', 'tjs': 'txiki'}
...
def eval_cmd(name, bin_path, script, iters, samples):
    ...
    subcommands (`tjs run script.js`); qzjs/node/bun take the script
    positionally."""
...
def run_js_api(bins, progress):
    ...
    qzjs needs --iters 0.25 (interpreter — the JIT runtimes use 1.0).
    ...
    for name, bin_path in bins.items():
        iters = '0.25' if name == 'qzjs' else '1.0'
        timeout = 900 if name == 'qzjs' else 300
...
def main():
    ...
    ap.add_argument('--bins', required=True,
                    help='comma-separated runtime=path entries '
                         '(qzjs,node,bun,tjs); missing ones skipped')
    ...
    if 'qzjs' not in res:
        print('FAIL: no qzjs numbers (required for ratios)', file=sys.stderr)
        return 1

    # Ratios vs qzjs: >1 means "slower/larger" for latency/memory, "faster"
    # for throughput.
    vs = {}
    q = res['qzjs']
    ...
    if args.js_api:
        ...
        if js_api:
            # per-metric ratio table: runtime / qzjs (>1 = "more")
            q = js_api.get('qzjs', {})
            js_api_ratios = {}
