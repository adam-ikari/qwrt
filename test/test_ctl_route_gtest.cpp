...
TEST(ctl_route, decide_local_when_target_is_self) {
    ...
    EXPECT_EQ(AM_CTL_ROUTE_LOCAL, am_ctl_route_decide(1, 1));   /* 主RT */
    EXPECT_EQ(AM_CTL_ROUTE_LOCAL, am_ctl_route_decide(5, 5));   /* worker */
    EXPECT_EQ(AM_CTL_ROUTE_LOCAL, am_ctl_route_decide(0, 0));   /* 宿主 */
}

TEST(ctl_route, decide_up_for_rootward_targets) {
    /* 0/1 且非本地 → 朝根方向上行（父/宿主）。 */
    EXPECT_EQ(AM_CTL_ROUTE_UP, am_ctl_route_decide(1, 0));
    EXPECT_EQ(AM_CTL_ROUTE_UP, am_ctl_route_decide(5, 0));
    EXPECT_EQ(AM_CTL_ROUTE_UP, am_ctl_route_decide(5, 1));
    EXPECT_EQ(AM_CTL_ROUTE_UP, am_ctl_route_decide(7, 1));
}

TEST(ctl_route, decide_down_for_child_slot) {
    /* >1 且非本地 → 下行到本地子槽位（多跳路径编码不做：单跳相对地址）。 */
    EXPECT_EQ(AM_CTL_ROUTE_DOWN, am_ctl_route_decide(1, 2));
    EXPECT_EQ(AM_CTL_ROUTE_DOWN, am_ctl_route_decide(1, 5));
    EXPECT_EQ(AM_CTL_ROUTE_DOWN, am_ctl_route_decide(3, 9));
}

TEST(ctl_route, decide_drop_for_invalid_target) {
    /* 负数 target：无此地址（§4.3 明确不做 -1 广播）。 */
    EXPECT_EQ(AM_CTL_ROUTE_DROP, am_ctl_route_decide(1, -1));
    EXPECT_EQ(AM_CTL_ROUTE_DROP, am_ctl_route_decide(5, -3));
}
...
TEST(ctl_route, three_level_request_reaches_worker_slot) {
    ...
    const int32_t t = 5;
    EXPECT_EQ(AM_CTL_ROUTE_DOWN, am_ctl_route_decide(1, t));
    EXPECT_EQ(AM_CTL_ROUTE_LOCAL, am_ctl_route_decide(5, t));
}
...
TEST(ctl_route, cmd_target_field_defaults_to_self) {
    ...
    EXPECT_EQ(7, am_ctl_cmd_target(with_t, strlen(with_t)));
    const char *without = "{\"op\":\"metrics\",\"correl\":\"x\"}";
    EXPECT_EQ(1, am_ctl_cmd_target(without, strlen(without)));  /* 缺省 1=主RT */
    const char *bad = "{not json";
    EXPECT_EQ(1, am_ctl_cmd_target(bad, strlen(bad)));
}
...
TEST(ctl_route, off_plane_drops_inbound_envelope) {
    HostCtx *h = host_create_ctl_plane(AM_CONTROL_OFF);
    ASSERT_NE(nullptr, h);
    const char *cmd = "{\"op\":\"metrics\",\"correl\":\"off-1\"}";
    EXPECT_EQ(-1, am_control_route(h->rt, 1, 0, 1,
                                     (const uint8_t *)cmd,
                                     (uint32_t)strlen(cmd)));
    host_destroy(h);
}
...
TEST(ctl_route, local_hit_dispatches_command) {
    ...
    EXPECT_EQ(0, am_control_route(h->rt, 1, 0, 1,
                                    (const uint8_t *)cmd,
                                    (uint32_t)strlen(cmd)));
 ...
 * 后续各跳由 am_control_route 按各节点自身深度重写。无 target_path 时保持
 * 旧单跳相对语义，扁平拓扑零改动。 */
...
TEST(ctl_route, cmd_target_prefers_target_path_head) {
    ...
    EXPECT_EQ(1001, am_ctl_cmd_target(cmd, strlen(cmd)));
}
...
TEST(ctl_route, cmd_target_falls_back_to_scalar_target) {
    ...
    EXPECT_EQ(1, am_ctl_cmd_target(no_t, strlen(no_t)));   /* 缺省 = 主RT */
    EXPECT_EQ(1, am_ctl_cmd_target(nullptr, 0));
}
