...
# 运行前先把 sw-sw3.js 重置为版本 A（上一次运行可能把它改写为 B）。
# 期望输出逐行匹配。用法: bash test/service_worker_sw3_e2e.sh <path-to-qwrt>
set -u
QWRT="${1:-./build_grpc2/qwrt}"
DIR="$(cd "$(dirname "$0")/sw-e2e" && pwd)"

cp "$DIR/sw-sw3-a.js" "$DIR/sw-sw3.js"

OUT="$(timeout 20 "$QWRT" "$DIR/main-sw3.js" 2>&1)"
...
SW3-B install
SW3-B activate
p3: during=A ctrl=activated
p4: after=B ctrl=activated cc=2
