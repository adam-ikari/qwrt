// test_context_gtest.cpp — Task 5: 多上下文 JS API（qzContext）
//
// 宿主只见主 context；子 context 由 qzContext.spawn 建立（返回 ctx id），
...
TEST(context_, spawn_isolation) {
    ...
    std::string spawnCode =
        "JSON.stringify(qzContext.spawn(" + JSON_string("globalThis.y = 5;") + "))";
...
TEST(context_, spawn_destroy_reuse_slot) {
    ...
    ASSERT_TRUE(host_value(h,
        "qzContext.destroy(1); JSON.stringify(qzContext.spawn('globalThis.b = 2;'))", &out));
    EXPECT_EQ("1", out) << "got: " << out;
...
// 挂起正在执行的主 context（active=0）被拒（QZ_ERR_BUSY）——不能销毁/挂起
...
TEST(context_, cannot_suspend_active_ctx) {
    ...
    ASSERT_TRUE(host_eval(h,
        "var rejected = false;"
        "try { qzContext.suspend(0, ''); } catch (e) { rejected = true; }"
