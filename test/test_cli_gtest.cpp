// test_cli_gtest.cpp — qzjs CLI 端到端（fork 子进程断言 stdout/stderr/退出码）
//
// 驱动真实的 qzjs 可执行文件（QZ_CLI_BIN），覆盖：
...
TEST(CliTest, HelloWorld) {
    auto r = run_cli({"-e", "console.log('hello from qzjs')"});
    EXPECT_EQ(0, r.exit_code);
    EXPECT_NE(std::string::npos, r.out.find("hello from qzjs"));
}
...
TEST(CliTest, VersionPrintsVersion) {
    ...
    EXPECT_EQ(0, r.exit_code);
    EXPECT_NE(std::string::npos, r.out.find("qzjs"));
}
