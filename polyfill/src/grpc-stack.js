/**
 * amoib polyfill: the gRPC/HTTP2 stack, as one swappable wiring unit.
 *
 * `index.js` imports this through the virtual specifier `@amoib/grpc-stack`,
 * which polyfill/build.js aliases here when AM_WITH_GRPC=1 and to
 ...
 *
 * Serialization policy: protobuf is the only codec, so a amoib client talks to
 ...
 * "注册惰性组"。grpc / protobuf / amoib.http2 三个挂载面共享一次 setup——
 * 首次访问其中任一触发 5 个子 setup（保持既有顺序）。
 *
 * AM_WITH_GRPC=OFF 时 index.js 引的是 grpc-stack-stub.js（空函数）→ 本
