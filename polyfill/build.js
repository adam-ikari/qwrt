...
if (isWatch) {
  ...
  try {
    // WinterTC polyfill bundle → src/polyfill_default.c (separate .c file so
    // context.c doesn't recompile on every polyfill change).
    ...
    // Worker boot shim — standalone IIFE (polyfill/src/worker-boot.js), no
    // esbuild bundling needed; compiled straight to bytecode →
