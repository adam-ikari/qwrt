/* qwrt example: gRPC 四形态自连（unary / 服务端流式 / 客户端流式 / 双向流式）
 *
 ...
 *
 * 运行（仓库根；需要 QWRT_WITH_GRPC=ON 构建，example 本身不编入默认构建）：
 *   ./build_grpc/qwrt examples/grpc-hello/grpc-hello.js
 *
 * 依赖的能力（qwrt 内置）：
...
(async function () {
  /* unary：Echo */
  var echo = await ch.invoke(Echo, { text: 'hi qwrt' });
  check('unary Echo', echo, { text: 'echo: hi qwrt' });
