/* qzjs example: full-featured HTTP server
 *
 * 演示在应用层用 qzjs 提供的能力搭建一个完整的 HTTP 服务器
 * （qzjs 只提供监听/回复、读文件、压缩的能力，其余由应用层实现）：
 *
 ...
 *   qzjs.fs.readFileBinary(path)      — 二进制读文件
...
function loadFile(filePath) {
  ...
  return qzjs.fs.readFileBinary(filePath).then(function(ab) {
...
/* ── 启动 ── */
console.log('qzjs example http server starting on http://127.0.0.1:' + PORT);
...
var srv = serve({ port: PORT }, async function(req) {
  ...
  if (pathname === '/api/hello') {
    return jsonResponse({ message: 'hello from qzjs', method: req.method });
  }
