...
typedef struct am_config_s {
    /* 主 context 启动时在 amoib 线程上 eval；抛异常 → am_create 返回 NULL */
    const char *initial_script;
    /* 出站消息回调：跑在 amoib 线程，必须线程安全。nullptr 表示宿主不接收消息。 */
    void (*message_cb)(am_t *rt, const char *json, size_t len, void *data);
    int  debug;                      /* 沿用 DAP bit 语义 */
    void *host_data;                 /* per-runtime opaque ptr，可经 am_get_runtime_data 读取 */
    /* 控制面三档（CTL-0/CTL-2）：OFF（默认，am_control 恒 -1）/
     * IN_PROC（进程内宿主线程命令）/ LOCAL（IN_PROC + 本机 uv_pipe 端点）。
     * 见 docs/plans/2026-09-04-control-plane-design.md §4.1。 */
    int control_plane;               /* am_control_plane_t 值 */
    /* LOCAL 档的端点路径（AF_UNIX）。NULL → 缺省 /tmp/amoib-<pid>-<n>.ctl。
     * 端点文件权限 0600；连接方以 SO_PEERCRED 校验 uid（§2.3 / §4.2）。 */
    const char *control_pipe_path;
    /* Worker 执行后端（M-P1/M-P2 多进程模型 §1.4）。取值语义随编译模型
     * （AM_PROCESS_MODEL）条件编译——见下方 am_worker_backend_t 注释。
     * 粒度 per-rt：同一 am_t 的全部 worker 同后端。M-P1 缺省 THREAD；
     * M-P2 起 ISOLATED 编译缺省 PROCESS（编译模型驱动缺省，§1.4）。 */
    int worker_backend;              /* am_worker_backend_t 值 */
} am_config_t;
typedef enum {
    AM_CONTROL_OFF = 0,     /* 默认：控制面关闭 */
    AM_CONTROL_IN_PROC = 1, /* 进程内命令（msgq 路径） */
    AM_CONTROL_LOCAL = 2,   /* IN_PROC + uv_pipe 本地端点（CTL-2） */
} am_control_plane_t;

/* Worker 执行后端（am_config_t.worker_backend 取值）。
 *
 ...
 *   mock 测试构建（AM_USE_MOCK_LIBUV，无 ipc 后端）：恒按 THREAD 排列
 * 铁律：只用符号常量，勿硬编码 0/1。消费方必须与库看到同一 AM_PROCESS_MODEL_*
 * 宏——CMake 构建经 target_compile_definitions(amoib PUBLIC …) 自动传递。 */
#if defined(AM_PROCESS_MODEL_ISOLATED) && !defined(AM_USE_MOCK_LIBUV)
typedef enum {
    AM_WORKER_BACKEND_PROCESS = 0, /* 编译缺省：独立进程 worker（M-P1 机制） */
    AM_WORKER_BACKEND_THREAD  = 1, /* 显式回退：真线程 */
} am_worker_backend_t;
#else
typedef enum {
    AM_WORKER_BACKEND_THREAD  = 0, /* 缺省：真线程（THREAD 编译 / mock 测试构建） */
    AM_WORKER_BACKEND_PROCESS = 1, /* 独立进程 worker（THREAD 编译未启用 → 报错） */
} am_worker_backend_t;
#endif
 ...
 * am_destroy (call one or the other, never both). After this returns the
 * runtime is torn down and must not be used (no further post_message).
 * Do NOT call am_destroy after this: destroy forces shutdown and would
 ...
 * 命令由 amoib 线程在自己事件循环的安全点自主执行；结果经 message_cb 异步
...
typedef struct am_ext_t am_ext_t;
