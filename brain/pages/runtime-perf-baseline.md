...
## 跨运行时对比（qzjs vs Node/Bun/Txiki）——**历史参考**（2026-09-09 本机 Ryzen 5800H / build_rel Release，一次性测量；权威跨运行时基线见 CI cross-runtime job（node/bun））
- 驱动：test/bench_cross_runtime.py（新增）+ test/bench/runtime/bench-eval-cross.js（同一 JS 代码 qzjs/node/bun/tjs 四运行时共用）；指标 R1 冷启动 / R6 eval 吞吐（1M iter）/ R5 进程峰值 VmHWM。设计文档 §8。
- 数值表（R1 ms / R6 M ops/s / R5 KB）：
  - qzjs：15.88 / int 28.2 · closure 12.9 · str 8.7 / 14,580
...
- 比值 vs qzjs：启动 node 2.01×（bun/tjs ~1.00×）；int/closure 吞吐 node 37/81×、bun 52/71×（str 仅 2.7/6.2×）；RSS node 12.6×、bun 11.2×；tjs 全轴 ~1×（同 QuickJS 引擎族，RSS 0.73× 反而更小）。两轮完整复跑一致（±10% 内；bun R1 双峰 15.8~32ms，复跑中位数落 15.8ms）
- 结论：**启动** qzjs≈tjs≈bun（15.8~15.9ms），node 2× 慢；**内存是最大差异化优势**（qzjs 14.6MB vs node/bun 160-183MB，11-13×）；**吞吐** JIT 快 37-81×，str 差距最小 2.7-6.2×；tjs 同引擎互证（1.15-1.34×）。qzjs/Txiki 定位「够用吞吐 + 极小内存/快启动」。
...
- time: 2026-09-09T00:14:44
  kind: decision
  summary: "追加跨运行时对比段（qzjs vs Node/Bun/Txiki，2026-09-09 本机实测）"
  source: "cross-runtime-bench session (bench_cross_runtime.py)"
  affects: [runtime-perf-baseline]

- time: 2026-09-09T00:14:59
  kind: decision
  summary: "跨运行时首测落档（2026-09-09 本机 Ryzen 5800H，qzjs build_rel Release）：R1 15.88ms / node 31.98（2.01×）、bun 15.84、tjs 15.87；R6 int qzjs 28.2 vs node 1042（37×）bun 1468（52×）tjs 36.6（1.3×）；R5 qzjs 14.6MB vs node 183MB（12.6×）bun 164MB（11.2×）tjs 10.7MB（0.73×）——内存最大差异化优势，启动三系并列，吞吐 JIT 快 37-81×（str 仅 2.7-6.2×）。Txiki 需 gcc-12 源码构建，暂不入 CI"
