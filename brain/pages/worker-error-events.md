...
- worker 侧：`am_worker_notify_error`（src/worker.c）在 worker 自己的 JSRuntime 内构造 `new ErrorEvent('error', {message, filename:'', lineno:0, colno:0, error:<Error>})` 并 `globalThis.dispatchEvent` → 触发 `self.onerror` / `addEventListener('error')`。
- 父侧：C 层仍经 postMessage 发 `{type:'error', error:<msg>}`（结构化克隆字节）；polyfill worker.js 的 `__am_dispatch__` 按 `d.type === 'error'` 路由到 `w._onerror`。**注意：父侧收到的是 MessageEvent，`e.data = {type:'error', error:<msg>}`，不是 ErrorEvent 实例**（计划目标文本说"均收到 ErrorEvent"，但计划代码片段与测试用 `e.data`，实现以片段+测试为准）。
