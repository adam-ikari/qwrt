...
- io_uring workaround 已**纳入 deps/libuv-c99-atomics.patch**（与 c99-atomics 合一，patch 文件 SSOT）：删除上游 `uv__use_io_uring` 中 `!SQPOLL → return 1` 无条件启用分支，使 `UV_USE_IO_URING=0`（qzjs.c setenv）在所有路径生效。
