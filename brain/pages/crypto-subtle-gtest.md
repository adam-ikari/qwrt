...
- crypto.subtle 由 ext_crypto.c（QZ_WITH_CRYPTO_EXT=ON）init hook 安装到全局；所有操作返回 Promise，测试用全局变量 + host_poll_until_value 轮询。
- 测试通过 add_qz_gtest(test_crypto_subtle_gtest) 注册，QZ_WITH_CRYPTO_EXT 条件守卫。
