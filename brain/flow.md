...
sequenceDiagram
  ...
  participant Q as amoib thread
  participant L as libuv
  participant J as QuickJS runtime
  H->>Q: am_create(cfg) — initial_script
  Q->>L: uv_run loop
  Q->>J: eval initial_script
  J->>J: postMessage({ok:true})
  J->>L: register pending timers/io
  Q-->>H: message_cb fired
  H->>Q: am_post_message({\"cmd\":\"eval\",\"code\":\"...\"})
  Q->>J: __am_dispatch__ → onmessage eval
  J->>J: run code, postMessage result
  Q-->>H: message_cb returns result
  H->>Q: am_destroy() — request stop
  Q->>L: uv_stop + join
```

## Other important flows

- Worker 生命周期：amoib 内部新建线程 → QuickJS 独立 JSRuntime → __am_dispatch__ 消息通道
- MessagePort 跨线程：pal.portCreate() 分配全局 port id 对 → 序列化到字节通 → 接收侧 __am_port_from_ref__ 重建代理
