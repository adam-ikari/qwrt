...
amoib 是 C99 编写的嵌入式 JS 运行时包装层：宿主通过 C API（am_create/destroy/eval/tick）
与 amoib 交互；amoib 拥有独立内部线程 + libuv 事件循环，polyfill（ES module 经 esbuild 打包）
...
```mermaid
graph TD
  H[Host C application] -->|am_* C API| R[amoib core<br/>src/amoib.c + bridge.c]
