...
qzjs 是 C99 编写的嵌入式 JS 运行时包装层：宿主通过 C API（qz_create/destroy/eval/tick）
与 qzjs 交互；qzjs 拥有独立内部线程 + libuv 事件循环，polyfill（ES module 经 esbuild 打包）
